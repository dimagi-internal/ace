...
describe('CHECKS array', () => {
  test('exports fifteen checks in canonical order', () => {
    ...
    expect(ids).toEqual([
      ...
      // other check's result, so it must not be buried at position 15.
...
// checkRenderedFromTemplate
//
// Regression fixture is the REAL failure: turmeric-market-study/20260828-1108
// shipped a plain synthesized Google Doc as its work order and this suite
// returned `verdict: pass`, 8/8, with the degradation recorded only as prose.
// The excerpt below is verbatim from that artifact.
// ---------------------------------------------------------------------------

/** Verbatim head of the plain doc that shipped on turmeric-market-study. */
const SYNTHESIZED_WO = [
  'WORK ORDER',
  '',
  'WO Number:        [WO Number — to be assigned under MSA]',
  'Opportunity:        Turmeric Market Survey',
  'Date:        2026-08-08',
  'Period of Performance        2026-08-11 to 2026-09-08',
  '',
  'This Work Order is annexed to the Master Services Agreement (MSA) between Dimagi, Inc. ' +
    '("Dimagi") and [Partner Name] (henceforth, referred to as "partner").',
  '',
  'NOTE (render provenance): This Work Order was generated by ACE Phase 1 (pdd-to-work-order) ' +
    'during an autonomous run. The styled Google Docs template (WORK_ORDER_TEMPLATE_ID) was not ' +
    'accessible to the service account at render time, so this document was produced directly ' +
    'from the synthesized contractual content rather than the branded template. Content is ' +
    'complete and reviewable; visual styling (branded tables, bullet formatting) should be ' +
    're-applied when the template is restored.',
  '',
  '1. Background',
].join('\r\n');

/** A template-rendered work order carries boilerplate with no tokens behind it. */
const TEMPLATE_RENDERED_WO = [
  'Work Order Agreement #WO-2026-014',
  'Turmeric Market Survey',
  'Work Order Number',
  '\tWO-2026-014',
  '10. Signatures',
  "IN WITNESS WHEREOF, the parties hereto have caused this Work Order to be executed by their " +
    "authorized agents as of the date first above written, and annexed to the parties' Master " +
    'Services Agreement (MSA) dated __________________.',
  'Subcontractor\tDimagi, Inc.',
  'Name: Amara Okafor\tName: Lucina Tse',
  'Title: Director\tTitle: COO Dimagi, Inc.',
  '245 Main Street, 2nd Floor,',
  'Cambridge, MA 02142',
  '(617) 649-2214; legal@dimagi.com',
  'Cambridge, Massachusetts, USA ｜ Cape Town, South Africa ｜ New Delhi, India ｜ www.dimagi.com',
].join('\r\n');

describe('checkRenderedFromTemplate', () => {
  test('passes a template-rendered work order', () => {
    const r = checkRenderedFromTemplate(TEMPLATE_RENDERED_WO);
    expect(r.pass).toBe(true);
  });

  test('FAILS the real turmeric plain doc that previously scored 8/8', () => {
    const r = checkRenderedFromTemplate(SYNTHESIZED_WO);
    expect(r.pass).toBe(false);
    expect(r.detail).toMatch(/NOT rendered from the branded template/);
  });

  test('names the confession it found', () => {
    const r = checkRenderedFromTemplate(SYNTHESIZED_WO);
    expect(r.detail).toMatch(/render provenance/i);
    expect(r.detail).toMatch(/service-account access excuse/);
  });

  test('tells the fixer NOT to just delete the note', () => {
    // The tempting wrong fix: strip the provenance paragraph and re-run QA
    // green while the document is still an unbranded plain doc.
    const r = checkRenderedFromTemplate(SYNTHESIZED_WO);
    expect(r.auto_fix_hint).toMatch(/do NOT edit the note out/);
    expect(r.auto_fix_hint).toMatch(/the note is a symptom/);
  });

  test('catches a SILENT fallback — one that does not confess', () => {
    // The durable half. Strip every confession phrase from the plain doc and
    // it must still fail, on missing template boilerplate alone. Without this,
    // the check would only ever catch a fallback honest enough to admit itself.
    const silent = SYNTHESIZED_WO.split('\r\n')
      .filter((l) => !l.startsWith('NOTE (render provenance)'))
      .join('\r\n');
    expect(/render\s+provenance/i.test(silent)).toBe(false);
    const r = checkRenderedFromTemplate(silent);
    expect(r.pass).toBe(false);
    expect(r.detail).toMatch(/template-only element/);
  });

  test('lists which template elements are missing', () => {
    const r = checkRenderedFromTemplate('1. Background\r\nSome prose.\r\n10. Signatures');
    expect(r.pass).toBe(false);
    expect(r.detail).toMatch(/Dimagi signatory block/);
    expect(r.detail).toMatch(/Dimagi legal contact line/);
  });

  test('stays export-format independent — the suite invariant (ace#1609)', () => {
    // The office footer was rejected as a fingerprint because it is present in
    // the plain-text fixture and absent from the markdown export of the SAME
    // document. This pins that reasoning: the two chosen fingerprints must
    // agree across both export shapes of a signatures table.
    const plain = 'Subcontractor\tDimagi, Inc.\r\nName: Lucina Tse\r\n(617) 649-2214; legal@dimagi.com';
    const markdown = '| Subcontractor | Dimagi, Inc. |\n| Name: Lucina Tse | (617) 649-2214; legal@dimagi.com |';
    expect(checkRenderedFromTemplate(plain).pass).toBe(true);
    expect(checkRenderedFromTemplate(markdown).pass).toBe(true);
  });

  test('points at the doctor probe rather than guessing at the cause', () => {
    const r = checkRenderedFromTemplate(SYNTHESIZED_WO);
    expect(r.auto_fix_hint).toMatch(/work_order_template_copyable/);
  });

  test('is registered in CHECKS and runs first', () => {
    // Ordering matters for the operator reading a failure: "this is not a
