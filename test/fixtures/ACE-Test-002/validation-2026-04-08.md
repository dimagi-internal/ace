# ACE-Test-002 — Walk-Through Validation (2026-04-08)

This is a manual walk-through validation of the focus-group framework changes (jjackson/ace#4) against the ACE-Test-002 fixture. I read each archetype-aware SKILL.md and simulated executing it against the fixture's files, then compared the simulated output to the regression spec in the fixture's `README.md`.
...
**Bottom line:** 5 of 7 archetype-aware skills can be validated against the fixture in its current form. The remaining 2 (`flw-data-review`, `cycle-grade`) need runtime data — either fake completed sessions and a closed-out opportunity state, or a separate "completed" companion fixture. These gaps existed in ACE-Test-001 too and predate this PR; they're noted at the bottom of this report under **Known fixture limitations**.

## Fixture additions made during validation

These were missing inputs that prevented end-to-end runnability. Added to **both** ACE-Test-001 and ACE-Test-002 since the gaps were pre-existing and symmetric:
...
**Expected output:** an PDD structurally equivalent to `test/fixtures/ACE-Test-002/pdd.md`, with all 5 stress-test checks passing (the fixture was constructed to pass).
...
- Step 4 (workaround active — Nova bot API not built) writes the brief to `ACE/ACE-Test-002/app-briefs/learn-app-brief.md`
...
**Recommendation:** Create a `test/fixtures/ACE-Test-002-completed/` companion fixture with 6 fake session-output records. Out of scope for this PR; logged as a follow-up.
...
**Why this is a partial:** Validating `cycle-grade` requires the fixture to be in a "completed" state with all the artifacts the grader reads. The fixture is in a "pending" state (per state.yaml). This is the same gap ACE-Test-001 has — neither fixture is currently set up for end-of-cycle skill testing.
...
These gaps existed in ACE-Test-001 before this PR and were inherited symmetrically by ACE-Test-002:
...
| No "completed-state" companion fixture | `cycle-grade`, `learnings-summary`, `opp-closeout`, `llo-feedback` | L | Would need a `ACE-Test-001-completed` and `ACE-Test-002-completed` with all the post-execution artifacts. Or add a "post-run" mode to the existing fixtures. |
