...
**Provenance on a preserved artifact names the dispatch that PRODUCED it, not the one that harvested it** ([#2237](https://github.com/dimagi-internal/ace/issues/2237)). The stamper runs over everything in the dir, and `00-*` / `*-FAILURE.*` survive the wipe by design — so it used to overwrite the one file class guaranteed to be a leftover with the current dispatch's id, version and serial, and the comparison the stamp exists for always answered "fresh". Now mtime decides: an artifact older than the dispatch keeps its original sidecar (or, with none, `dispatch_id: unknown-prior-dispatch` and `ace_version: unknown` — never invented context) and gains `carried_over: true` + `superseded_by: <harvesting dispatch>`; anything the dispatch actually wrote, including a `*-FAILURE.png` it re-captured over a preserved one, is stamped as its own. A consumer asks `isCarryover(prov, currentDispatchId)` rather than re-deriving the comparison. Implementation: `lib/screenshot-provenance.ts` (`resolveArtifactProvenance`, `stampArtifactProvenance`); tests: `test/mcp/mobile/failure-forensics-provenance.test.ts`.
...
**The boot is serialised per AVD, and the wait is bounded (ace#1821).** Two sessions used to be able to select and cold-boot the SAME AVD at once: `held` is derived from `ps`, and a peer that has decided to boot but not yet spawned its qemu is in no process table. `ensureAvdRunning` now takes a per-AVD **boot claim** (`~/.ace/sessions/.avd-boot-<avd>.json`, `O_EXCL`) before it touches the AVD, and releases it the moment the emulator registers with adb — at which point the `ps` signal takes over. Three tiers, in order: **fall through** (a live claim is folded into `AvdCandidate.held`, so the existing `resolveAvdPoolFreedom` → `selectAvd` pipeline just picks another pooled AVD — zero wait, which is why `--pool 2` is worth running); **wait**, bounded by 1.25× the adb-register budget, when every AVD is claimed; then **proceed anyway**, taking the claim over. It never refuses — a refusing lock on a one-AVD host converts "limps to a partial walk" into "dies immediately" while Phase 6's one-way Learn precondition is consumed either way. Staleness has two tests, not one: dead owner (as `withAllocatorMutex`) **and** age, because a live-but-wedged owner would otherwise hold the claim forever. Decision logic + the full argument: `lib/avd-boot-claim.ts`; plumbing beside `withAllocatorMutex` in `mcp/mobile/session-lock.ts`. **Per-`$HOME`, so it does not serialise across macOS accounts** — cross-account contention is still only *visible* (`lib/mobile-contention.ts`), not excluded.
