...
Phases 1–7 run end-to-end with zero LLO involvement. Phase 8 publishes a public solicitation (and emails PDD-named candidates if any). Phase 9 is the first 1-1 LLO contact, gated on `phases.solicitation-management.products.selected_llo.org_slug` in the current run's `run_state.yaml`. Phase 6 splits shallow (in `/ace:run`, ~5 LLM judges) vs deep (out-of-band via `/ace:qa-deep`, ~90 judges); `llo-launch` requires fresh deep verdicts. `app-multimedia-coverage` is a manual post-Phase-3 step invoked via `/ace:step`. (Sibling skill `commcare-form-patch` was quarantined 2026-05-22 — see `skills/commcare-form-patch/SKILL.md`; Nova maintainer #7 says wrappers are required, ACE's auto-strip in Step 2.8 disabled pending live re-verification.)
