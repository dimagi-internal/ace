...
- **Orchestration runs 7 phases as of 0.10.0.** Phase order: (1) design-review → (2) commcare-setup → (3) connect-setup → (4) ocs-setup → (5) qa-and-training → (6) llo-manager → (7) closeout. The new Phase 5 `qa-and-training` agent synthesizes upstream artifacts into screenshots (`app-screenshot-capture`) and training docs (`training-materials`, relocated from Phase 2) before any LLO contact, restoring the "Phases 1–N agent-only, then LLO contact" invariant. Phase 6 is the first LLO-facing phase.
