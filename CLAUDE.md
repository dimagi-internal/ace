...
Phases 1–6 run end-to-end with zero LLO involvement. Phase 7 publishes a public solicitation (and emails PDD-named candidates if any). Phase 8 is the first 1-1 LLO contact, gated on `phases.solicitation-management.outputs.selected_llo.org_slug` in the current run's `run_state.yaml` (legacy fallback: `opp.yaml.selected_llo.org_slug` until state-consolidation cleanup PR e). Phase 5 splits shallow (in `/ace:run`, ~5 LLM judges) vs deep (out-of-band via `/ace:qa-deep`, ~90 judges); `llo-launch` requires fresh deep verdicts. `app-multimedia-coverage` is a manual post-Phase-2 sibling of `commcare-form-patch`, invoked via `/ace:step`, not part of `/ace:run`.
...
This repo is dogfooded by the `canopy` plugin. **Per-run evidence lives in Drive at `ACE/<opp>/runs/<run-id>/`: `run_state.yaml` (now the source of truth for every evolving piece of state — Connect IDs, OCS chatbot, solicitation, selected_llo, synthetic — under `phases.<phase>.outputs.*`) plus per-phase `<N>-<phase>/<skill>_*` files (verdicts, transcripts, comms-logs). Cross-run inheritance happens once, at run init, when the orchestrator seeds the new run's `run_state.yaml` from the prior run's outputs (see `agents/ace-orchestrator.md § Step 6b`). Per-opp state — shared across every run, never mutated by phase skills — lives at `ACE/<opp>/`: `opp.yaml` (thin identity: display_name, slug, tags, created_at, created_by), `inputs/`, `eval-calibration/known-issues.md`, `open-questions.md`. Cross-opp strategy lives in `.claude/pm/runs/<date>-<lens>.md`.** ACE skills don't read run logs; canopy and humans do. (State-consolidation PRs a-d landed 2026-05-10/11: `connect-state.yaml` retired; `opp.yaml.{connect, solicitation, selected_llo, synthetic, ocs_chatbot}` blocks moved to `run_state.yaml.phases.<phase>.outputs.*` with read-old-and-new fallbacks until cleanup PR e. `run_state.yaml` was renamed from `state.yaml` in 0.11.3; `gate-briefs/` removed 0.13.116; full per-opp vs per-run table + forking recipes in `agents/orchestrator-reference.md § Fork Points`; design doc at `docs/superpowers/specs/2026-05-10-state-consolidation.md`.)
