...
- **Reading email — canonical forms, don't re-derive them.** `canopy email read <threadId>` (structured, preferred); to find threads: `gog gmail search "<query>" -a $ACE_GMAIL_ACCOUNT --client $ACE_GMAIL_CLIENT --max N -j`; raw thread: `gog gmail thread get <threadId> -a $ACE_GMAIL_ACCOUNT --full`. The flag is `--max` (there is no `--limit`), search/thread-get need `-a <account>`, and **there is no `message get` subcommand** (`messages` only has `search`/`modify` — read a single message via its thread). Measured 2026-07-28..30: six sessions each burned ~5–10 calls rediscovering exactly this.
