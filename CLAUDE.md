...
- **For an external state transition, attempt the transition and treat the conflict as the skip — never let a read-back flag stand in for it.** Distinguish two kinds of "is this already done?" pre-check: (a) a **cost-skip** that avoids redundant work but is re-validated live downstream (`ocs-agent-setup` reads its run-local state file to skip a re-clone, but Step 10 still does a live embed round-trip) — fine; (b) a **correctness-skip** that reads one system's flag and *decides not to perform a state transition another path depends on* — a footgun, because the flag may not mean what you think. `connect-opp-setup` read the managed-opp create endpoint's side-effect `active: true` and skipped the `/activate/` call — but that flag is NOT the `/activate/` transition `invite_users/` requires, so invites failed (jjackson/ace#624). The safe shape: call the transition unconditionally and catch the "already done" error as the skip signal; branch on the *call result*, not a read-back flag. A correctness-skip is only safe when its guard is the very state the dependent step reads (or it re-validates live). Sibling of "restore, don't adapt" and "close the loop to the source of truth."
- **File ACE issues mid-run the moment you've confirmed one — don't just narrate.** Navigating around a broken script/recipe/atom to keep a run moving is expected. But once you've *confirmed* a defect or concrete improvement (root cause + how ACE should change), **immediately `gh issue create` against the ACE repo's `origin` (no `-R` needed)** — one issue per finding (title + root cause + run/repro + a "fix lands here: `path`" pointer) — then keep going. Don't defer to run-end (you'll lose the precise repro) or fix-in-place silently (the class-level fix gets lost). Bar: file when you'd bet it's real — false positives are cheap to close, lost findings are not. **Report every issue filed in the run summary.** Pairs with "class-level preventers": the issue is how the preventer gets tracked.
...
- **Granting access is a ROLE decision, not a membership decision — and HQ's "Read Only" role cannot open the app links ACE shares.** The stock HQ `Read Only` preset carries only `view_reports`/`download_reports` (`corehq/apps/users/role_utils.py`), while App Summary and the app-builder form/module views need `edit_apps` OR `view_apps` (`require_can_edit_or_view_apps`, `app_manager/decorators.py`). The releases page still renders for them (it's only `login_and_domain_required`), so access looks like it "mostly works" right up until every app link 403s. Only `App Editor` and `Admin` carry `view_apps`. `scripts/grant-review-access.ts` now defaults to `App Editor` and **reconciles an existing member's role** instead of short-circuiting on "already a web user" — a wrong role is NOT "already present". Same class as the OCS `Chat Viewer` 403 trap documented in that script's header. (Sophie Feintuch, 2026-07-23: invited Read Only, could see the app but 403'd on App Summary.) The OCS half reconciles the same way: an already-accepted team member on the wrong group also 403s on the linked chatbot page, so `grantOcs` edits the member's groups (additively — `MembershipForm` REPLACES the m2m set) via `members/<id>/` rather than skipping on "already a member". Both halves prove the write against a fresh read, never the POST status.
