...
Phases 1–6 run end-to-end with zero LLO involvement. Phase 7 publishes a public solicitation (and emails PDD-named candidates if any). Phase 8 is the first 1-1 LLO contact, gated on `phases.solicitation-management.outputs.selected_llo.org_slug` in the current run's `run_state.yaml`. Phase 5 splits shallow (in `/ace:run`, ~5 LLM judges) vs deep (out-of-band via `/ace:qa-deep`, ~90 judges); `llo-launch` requires fresh deep verdicts. `app-multimedia-coverage` is a manual post-Phase-2 sibling of `commcare-form-patch`, invoked via `/ace:step`, not part of `/ace:run`.
