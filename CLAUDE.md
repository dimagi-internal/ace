...
- **Nova plugin is a separate install.** Nova ships as its own Claude Code plugin (`voidcraft-labs/nova-marketplace`, slash commands like `/nova:autobuild`, `/nova:upload_to_hq`). ACE consumes Nova as a sibling plugin — install it once with `/plugin install nova@nova-marketplace`, sign in via OAuth on first use, and the `pdd-to-learn-app` / `pdd-to-deliver-app` / `app-deploy` skills will invoke its slash commands. There's no Nova MCP server in this repo. Active blocker as of 2026-04-27: `dimagi-ai.com` needs to be added to Nova's Google OAuth allowlist alongside `dimagi.com`. See `playbook/integrations/nova-integration.md`.
