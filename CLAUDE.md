...
- `mcp/` — 5 MCP servers wired inline in `.claude-plugin/plugin.json` `mcpServers` (inline to work around [anthropics/claude-code#9427](https://github.com/anthropics/claude-code/issues/9427)). Atom signatures live in `docs/atom-schemas.md` (generated, always-accurate) — grep there rather than trusting counts:
  ...
  - `ace-mobile` (`mobile-server.ts`) — Mac-local AVD + Maestro. Static recipe palette + `${SELECTOR:...}` map (default APK 2.63.0) + structural recipe-defect linting; full detail in `playbook/integrations/mobile-integration.md`. Source under `mcp/mobile/`.
...
- **Self-heal a filed issue when you can, then close it — don't just file and route around** (Jon, 2026-07-22). Filing is the floor, not the ceiling. The instant a filed issue is *self-healable* — you understand the root cause AND the fix is bounded, low-risk, and lands in the ACE repo (a recipe/palette/selector/skill/atom edit, a doc/contract fix) — **fix it now: edit → `bash scripts/version-bump.sh` → commit → PR → arm auto-merge (`gh pr merge <n> --auto --merge`) → `gh issue close <n>` referencing the PR → `/ace:update` → then re-run the step that was blocked.** Don't wait for run-end and don't leave a green issue tracking a fix you could have shipped in-session. **Two hard gates before you merge a self-heal:** (1) **Live-validate anything ACE's own rules say to** — a mobile recipe/selector fix MUST be proven on a live device (re-run the blocked leg to green) before it merges — export `ACE_MOBILE_STATIC_RECIPES_DIR=<repo>/mcp/mobile/recipes/static` and restart Claude to run the staged palette without patching the plugin cache (recipe: `playbook/integrations/mobile-integration.md § Validating a palette fix pre-merge`); "close the loop to the source of truth" is not waived by the self-heal convention, it's the precondition for it. An unvalidated fix reopens the same class. (2) **Scope discipline** — self-heal the surgical fix behind the issue; if the "fix" needs human product/taste judgment, a risky cross-cutting refactor, or you can't validate it this session, leave the issue open and say why. When you self-heal, the run summary reports the issue as *filed + fixed (PR #n, merged)*, not just filed.
...
- **Raw `adb` must target the mobile MCP's own adb server: `ANDROID_ADB_SERVER_PORT=5039`.** The default 5037 reports an empty device list while the emulator is running fine, which reads as a dead device. *Enforced:* `mobile_diagnose` now works on the local backend and reports the port + serial in use (`test/mcp/mobile/local-diagnose.test.ts`).
