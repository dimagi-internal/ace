...
- **Raw `adb` must target the mobile MCP's own adb server: `ANDROID_ADB_SERVER_PORT=5039`.** The default 5037 reports an empty device list while the emulator is running fine, which reads as a dead device. *Not enforced (`harness`, #961 — `mobile_diagnose` is cloud-only, so the local backend has no self-describing probe).*
