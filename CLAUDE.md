...
- **Close the loop to the source of truth — don't guess at what another system owns.** When a bug is really "ACE is guessing at a value another system is authoritative about," the fix is to *observe the real value from that system once* and encode it behind a test — never to ship another plausible guess. Two recurring shapes: (1) **live-device truth** — a selector map / Maestro recipe asserting resource-ids the running app actually renders; calibrate it against a live `mobile_capture_ui_dump` (and *run the recipe on-device to confirm it navigates*), don't transcribe from a sibling APK version or infer from convention. The 2.63.0 selector map drift (jjackson/ace#591/#593) is the canonical miss: it was copied unverified from 2.62.0 and only corrected reactively when Phase 6 burned wall-clock hitting each wrong selector. (2) **contract truth** — when an upstream validator/runtime (Nova's `validate_app`, Connect's budget check) is the authority, get the authoritative answer from that system rather than trial-and-erroring values through it (jjackson/ace#586's `entity_id` cycled three guesses, each failing a different gate). The discipline: a guess validated against the real system once, then locked behind a test, beats N reactive instance-fixes. And **when you change recipe/selector logic, validate it live before shipping** — unvalidated recipe branches recreate the very class you're fixing (a Proceed-branch guard bug was caught live on 2026-05-31 only because the corrected recipe was re-run on the device; it would have shipped broken otherwise). If you genuinely can't reach a surface this session, encode what evidence confirms and mark the residual precisely — don't fill the gap with a guess. New-APK-version maps should be calibrated against a live device as a first-class step, not discovered wrong in production.
