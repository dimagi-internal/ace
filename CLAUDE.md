...
- **Hooks are rails, not approval gates** (Jon, 2026-07-01). `hooks/gating_guard.py` (PreToolUse on `Bash`, plugin-level — fires in every session with ACE installed; rules in `config/gating.json` stay NARROW) carries **deny rails only**: raw `gog gmail send|reply` under the ACE identity is hard-blocked with a message naming `bin/ace-email` (the only send path — HTML multipart wrapper + threadId capture). The `approve` list is deliberately empty — interactive "ask" prompts stall autonomous runs (the hal lesson); approvals are procedural instead (pause-point mode matrix in `run_state.yaml`, `solicitation-review`'s HITL checkpoint, review posture in turns). Don't add ask rules; encode a new invariant as a deny rail + a procedural gate.
