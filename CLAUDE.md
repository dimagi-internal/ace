...
This repo is dogfooded by the `canopy` plugin. **Per-run evidence lives in Drive at `ACE/<opp>/runs/<run-id>/`: `run_state.yaml` plus per-phase `<N>-<phase>/<skill>_*` files (verdicts, transcripts, comms-logs). Per-opp state — shared across every run — lives at `ACE/<opp>/`: `opp.yaml`, `inputs/`, `eval-calibration/known-issues.md`, `open-questions.md`, `connect-state.yaml`. Cross-opp strategy lives in `.claude/pm/runs/<date>-<lens>.md`.** ACE skills don't read run logs; canopy and humans do. (`run_state.yaml` was renamed from `state.yaml` in 0.11.3; `gate-briefs/` removed 0.13.116; full per-opp vs per-run table + forking recipes in `agents/orchestrator-reference.md § Fork Points`.)
