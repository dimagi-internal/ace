...
and surfaced to LLOs via the onboarding email in Phase 5; they are not
attached to the Connect opportunity record itself today.

LLO invitation list preparation lives in Phase 5 (`llo-manager`) — we don't
...
- **Activation:** the opportunity is created in `draft` and stays there
  until `llo-launch` (Phase 5) flips it to `active` after UAT.
