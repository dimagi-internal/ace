...
| Connect opportunities with an EMPTY `end_date` | connect | ✅ Auto-repair via `connect_update_opportunity({end_date})` — a **repair, not a deletion**. An opp with no end date bricks the mobile app for every worker invited to it (ace#938; one found the hard way, ten more landmines in the same org). Two guards: only ever FILL an empty date, never shorten one; and use a FUTURE date for a live-set opp so the repair can't end a running opportunity. |
| Connect demo-user `OpportunityAccess` (pending) | connect | 🚧 Report-only, awaiting ace#1159 — `connect_delete_unaccepted_flw_invites` is uncallable because no atom returns the `UserInvite` row id. |
| Connect demo-user `OpportunityAccess` (accepted) | connect | ❌ Upstream gap, and **ace#1159 does NOT close it**. The delete view excludes `status=accepted` server-side, and every successful Phase 6 run claims its tile — so the accepted rows are the dominant class and are unreachable through that view whatever ids we obtain. `delete_opportunity()` has no view/REST/atom. Operator escape: rotate `${ACE_E2E_PHONE}`. Cost of leaving it: device tiles accumulate forever (the mobile list endpoint has no `active=True` filter, so soft-deactivate does not remove them) and Phase 6's claim scroll runs a FIXED budget against an unbounded list — already two deterministic failures at 8+ tiles deep (ace#647). |
