...
| `connect_preflight_learn_app_user` (CI-660) | `mcp/connect/backends/commcare-preflight.ts` (atom impl) + `mcp/connect-server.ts` (wiring); recommended caller `skills/connect-opp-setup/SKILL.md` Step 7.5 | Connect's `users/views.py:107 start_learn_app` doesn't wrap `create_hq_user_and_link` in try/except, so HQ-side rejections (rotated API key, archived domain, CCHQ outage, already-linked-to-different-ConnectID user) bubble out as opaque HTTP 500. The Android client logs+swallows the failed response, producing a silent "Start learning" button noop in Phase 5. Identical class to the `short_description` 50-char trap. | `docs/learnings/2026-05-12-ci-660-preflight.md` — graduates the Pending row originally filed from PR #249 (commit `8677225`). |
