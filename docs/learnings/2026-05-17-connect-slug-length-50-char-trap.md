...
| `connect.deliver_unit.entity_id` / `entity_name` are OPTIONAL XPath fields | `src/commcareConnect.js:240, 249` (`presence: 'optional'`) | Nova has them but the `update_form` deliver_unit runtime auto-fills broken defaults (`voidcraft-labs/nova-plugin#1`) — known upstream bug **at the time of writing; #1 closed COMPLETED 2026-04-29 and `update_form` now exposes `entity_id`/`entity_name` correctly (CLAUDE.md § Nova). Do not cite this row as a live constraint.** |
