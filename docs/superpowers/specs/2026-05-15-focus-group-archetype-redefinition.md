...
- **Structured `per_unit_payment` field on the labs solicitation schema.** Considered after Phase 4 surfaced "the $120/session figure lives in scope_of_work prose, not a structured field." Operator decided **against** pushing for a schema addition: per-unit payment shape varies by archetype (per-visit / per-session / per-stage); within each archetype the right rate is opp-and-LLO-specific; and the load-bearing economic interaction is the **negotiation** between the published range and the responding LLO's proposal, not a single declared number. Solicitations express payment as a range with rationale in `scope_of_work`, and the `questions` block asks the responding LLO to propose a number. See `solicitation-create/SKILL.md § Process` for the design principle.
...
- **2026-05-15** — Resolved the open "labs `per_unit_payment` schema gap" as **not a gap**. Operator verbatim: "we should not be trying to set a per-payment-unit level data model on solicitations, we can include it on the text but that is too specific and likely it would be a range in the solicitation that the org should respond to what rate they are proposing and why usually." Design principle codified in `solicitation-create/SKILL.md § Process`: solicitations express payment as a range with rationale in `scope_of_work` prose; the `questions` block asks the responding LLO to propose their actual rate + why; the awarded LLO's proposed rate becomes the `connect.deliver_unit` payment_unit amount at Phase 4 setup. `payment-rate` and `per-session-rate` Decisions Log catalog rows updated to "range, not fixed."
