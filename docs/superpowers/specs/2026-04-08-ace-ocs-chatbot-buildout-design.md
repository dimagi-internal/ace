...
Status annotations were added after the initial implementation pass shipped (2026-04-08).

1. **[PENDING]** **Clone channel copy behavior.** Does `experiment.create_new_version(is_copy=True)` carry `ExperimentChannel` rows? If not, `clone_chatbot` needs a post-clone step to create an `EMBEDDED_WIDGET` channel and capture its `widget_token`. The `getChatbotEmbedInfo` atom already throws a loud error if no channel is found, so failure is detectable — but the actual behavior still needs to be verified against a live clone. Verify by reading the rest of the `create_new_version` implementation or by running a test clone against OCS dev.
2. **[PARTIALLY RESOLVED]** **Embed key scrape path.** The `extractWidgetToken` helper in `mcp/ocs/backends/playwright.ts` uses a simple `data-widget-token="..."` regex. This is the first-guess implementation; the actual DOM on `/a/<team>/chatbots/<id>/channels/` has not been inspected. When a live session is available, walk through the OCS UI once by hand and confirm the DOM matches the regex — adjust if not.
...
4. **[RESOLVED]** **CSRF extraction method.** `extractCsrfToken` in `mcp/ocs/auth/playwright-session.ts` reads the `csrftoken` cookie and the `PlaywrightBackend` passes it as `X-CSRFToken` — both match Django's defaults. Unit-tested.
...
11. **[PENDING]** **`pipeline_id` discovery.** `PlaywrightBackend.pipelineIdFor` reads `pipeline_id` (or `pipeline.id`) from `GET /api/experiments/<id>/`. Whether the OCS public API actually exposes that field is still unverified — the backend throws a clear error referencing this item if the field is absent. If it is absent, the fallback is a small scrape of the chatbot home page.
...
13. **[PENDING]** **`uploadCollectionFiles` multipart.** The `add_collection_files` Django view (`apps/documents/views.py:352`) parses `request.FILES`, which expects real `multipart/form-data`. The current implementation in `PlaywrightBackend.uploadCollectionFiles` sends the files as a JSON body through the injected `RequestFn` — this is the plan-as-written and will likely fail on first contact with OCS. The unit test is intentionally loose and does not guard against this. Fix on first real-OCS run by routing through Playwright's native `multipart:` option (either extend `RequestFn` with a multipart channel or use a direct `page.request.post` call inside the atom). Tagged with a `TODO` in the atom source.
