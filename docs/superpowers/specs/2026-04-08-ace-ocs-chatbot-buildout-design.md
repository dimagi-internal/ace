...
1. **Clone channel copy behavior.** Does `experiment.create_new_version(is_copy=True)` carry `ExperimentChannel` rows? If not, `clone_chatbot` needs a post-clone step to create an `EMBEDDED_WIDGET` channel and capture its `widget_token`. Verify by reading the rest of the `create_new_version` implementation or by running a test clone against OCS dev.
2. **Embed key scrape path.** Exact DOM path / form field on `/a/<team>/chatbots/<id>/channels/` that carries the `widget_token`. Walk through the OCS UI once by hand and capture the structure.
3. **LLM provider and embedding model defaults.** What is configured on the target team? Needed for sensible defaults in `create_collection`. Probably "team's default LLM provider + default embedding model"; may need a `list_llm_providers` atom if defaults can't be hardcoded.
4. **CSRF extraction method.** Confirm Django sets `csrftoken` as a cookie and that `X-CSRFToken` header is accepted. Both are Django defaults; quick verification only.
5. **OpenAI Assistants status.** Jon flagged as likely deprecated. Confirm with Dimagi before v2 planning; no v1 action.
6. **Live Connect OCS bot inspection.** When Jon has access, pull the live pipeline via `GET /a/<team>/pipelines/data/<pipeline_id>/` and document: node topology, system prompt text, tools, Collection attachment, LLM provider + model, embedding model. Validates the golden-template-as-fork-of-Connect-bot approach and may surface a second LLM node that would break the invariant.
7. **Team slug.** Confirm whether ACE chatbots live under `dimagi`, `connect`, or a new team. Affects every URL constructed by the Playwright backend.
8. **Session state storage path.** `~/.ace/ocs-session-<team>.json` is the default. Confirm this works with Claude Code plugin sandbox permissions.
9. **Golden template creation as a prerequisite.** Someone (likely Jon manually, once) clones the live Connect bot, adds ACE-specific framing (escalation rules, per-opp placeholder prompt), and records the integer experiment ID as `OCS_GOLDEN_TEMPLATE_ID`. Documented here as a one-time manual bootstrap step.
10. **SSO flow for `chatbots.dimagi.com`.** If Dimagi SSO sits in front of OCS, scripted `OCS_USERNAME` + `OCS_PASSWORD` login is not possible; the fallback is a manually-captured `storageState.json`. Confirm the production auth shape.
11. **`pipeline_id` discovery.** `clone_chatbot` returns `pipeline_id` via `GET /api/experiments/<id>/`. Verify that endpoint actually includes the pipeline reference in its response body; if not, add a small scrape of the chatbot home page.
12. **Rate limits on the OCS web routes.** The Playwright backend hits Django views that weren't designed for programmatic use. Check whether there are any rate limits or request signatures that would flag the traffic. Probably none, but verify on a dev instance before running against production.
