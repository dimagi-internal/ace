...
Status annotations were added after the initial implementation pass shipped (2026-04-08), updated after the post-merge source-code verification pass (2026-04-09, AM), and updated again after the live-OCS verification pass (2026-04-09, PM) using a Dimagi sandbox team. Live-OCS validation exercised every critical authoring path end-to-end, caught two more bugs (form-encoding and the wrong-chatbot-id scrape), and bootstrapped a real golden template on that team (experiment id recorded in the local `.env`, not committed to the repo).
...
2. **[RESOLVED + LIVE-VERIFIED]** **Embed key scrape path.** The `widget_token` is rendered by `templates/channels/widgets/widget_params.html` as `<input type="text" id="widget_token" value="{{ widget.token }}" ...>`. The previous `data-widget-token="..."` regex was wrong. **Fix shipped:** `extractWidgetToken` regex updated to `/id="widget_token"[^>]*value="([^"]+)"/`. The template is embedded in the channel *edit-dialog* view, not the channels list — so `getChatbotEmbedInfo` now does a 3-hop scrape (home page → find channel_id from the channel button's `hx-get` URL → GET edit-dialog → scrape token). Live-verified 2026-04-09 against the bootstrapped golden template: scrape returned a real widget token.
3. **[PARTIALLY RESOLVED]** **LLM provider and embedding model defaults.** The `createCollection` atom accepts `llm_provider` and `embedding_model` as optional numeric IDs. On the sandbox team, the test account does not have permission to list LLM providers directly (`/a/<team>/service_providers/llm/` returns 404). **Workaround in place:** the golden-template-clone strategy means every per-opp chatbot inherits its LLM provider + model IDs from the golden template's pipeline (which in turn inherited them from the chatbot it was cloned from). `createCollection` is only needed for explicit collection creation, which a permission-constrained user can't do anyway. Fully resolving this requires either a user with `service_providers.view_llmprovider` permission, or a REST API that exposes provider IDs. Not blocking v1.
...
6. **[PARTIALLY RESOLVED]** **Live Connect OCS bot inspection.** Observed anonymously via its widget embed endpoint on 2026-04-09 (not via admin inspection — the test account doesn't have admin on the Connect bot's team). What we learned:
    - **Bot name**: "Connect Support Bot"
    - **7 published versions**, iterated on since the initial deploy; working version is v8. Version descriptions trace its evolution from "individual Confluence pages" to "public space rather than individual pages" (moved from page-level references to a whole Confluence space as a DocumentSource with auto-sync).
    - **Persona targets Network Managers** (NMs), not "LLOs". The ACE spec's "LLO" terminology is CRISPR-Connect-internal; production Connect uses NM. ACE's golden-template prompt should reconcile these.
    - **Pure RAG, no tool wiring.** The bot explicitly self-reports "I don't have access to live systems, personal data, or external websites" and "I use uploaded documents to answer your questions." No tools, custom actions, or MCP tools visible from the user-facing side.
    - **Confluence-sourced Collections.** Version descriptions explicitly mention "updated confluence pages", "added Managing credentials Confluence page in the Collections", "CLP and Vaccine related FAQs". Matches the spec's prediction that Collections + DocumentSource auto-sync is the modern RAG path.
    - **Concrete procedural knowledge**: gave multi-step answers for "review flagged deliveries" (referencing the Visit Verification Page, "Pending NM Review" filter, bulk approve with justification) and "add a payment unit" (mapping delivery units to CommCare forms, "Sync Deliver Units" button). The retrieval quality on specific Connect-app procedures is high.
    - **Generic escalation handling** — tells users to contact "your organization's designated support contact". No specific escalation inbox. ACE's per-opp bots should override this with an explicit `Ace-AI@Dimagi.com` escalation rule in the system prompt.
    - **Message response shape**: `{message: {created_at, role: "assistant", content, metadata: {trace_info, cited_files, generated_files}, tags, attachments}, status: "complete"}`. Tags include the version label (e.g. `"v7"`). `cited_files` was empty on every observed response — either the bot's LLM doesn't emit citations, or they only surface via a prompt convention we didn't observe.
    - **The production OCS team slug** (where the Connect bot lives and where ACE chatbots should also live) is recorded in the local `~/.ace/connect-ocs-bot.json`, not committed.
    **Conclusion for the golden-template design**: the Connect Support Bot is NOT a good base to clone for per-opportunity ACE bots (its knowledge base is Connect-wide, which would pollute per-opp context). It IS a good reference for tone, structure, and the Confluence-sourced Collection pattern. ACE's golden template should be a minimal new chatbot on the same OCS team (so admin users can manage both), with the ACE-specific prompt skeleton and an empty Collection that per-opp `ocs-agent-setup` flows populate.
    Full anonymous admin inspection (pipeline graph, actual system prompt, LLM provider, tool config) still requires team membership on the Connect bot's OCS team.
7. **[RESOLVED + LIVE-VERIFIED]** **Team slug.** Both sandbox and production team slugs have been identified — both recorded in local env files (`.env` / `~/.ace/connect-ocs-bot.json`), neither committed. The production team is where the Connect Support Bot currently lives, and where ACE's golden template + per-opp clones should also live so a single admin can manage both. Verification surfaced the prod team via the `trace_url` field (`/a/<team>/traces/<id>/`) in the bot's message metadata, observable via the anonymous widget-embed chat API.
...
9. **[RESOLVED]** **Golden template creation.** Automated via `scripts/bootstrap-ocs-golden-template.ts` and documented in `commands/ocs-bootstrap-template.md`. Bootstrap is idempotent (detects existing template by name) and supports `OCS_BOOTSTRAP_FORCE=1` for refresh-in-place. Live-bootstrapped on a Dimagi sandbox team 2026-04-09 — the resulting experiment id, public_id, and embed_key are recorded in the local `.env` (gitignored). The template was cloned from an existing chatbot on that team and patched with the ACE-flavored system prompt skeleton. Re-running the bootstrap for production requires access to a production OCS team (pending).
10. **[PARTIALLY RESOLVED]** **SSO flow for `chatbots.dimagi.com`.** The Dimagi sandbox test account uses standard Django username/password login — no SSO redirect — and the scripted headless login path via Playwright works (verified 2026-04-09: fills the form, submits, and lands at the team dashboard with a valid `sessionid` cookie). For production Dimagi SSO accounts the behavior may differ; `/ocs:login` (headed browser) remains the fallback for those cases.
11. **[RESOLVED + LIVE-VERIFIED]** **`pipeline_id` discovery.** The OCS REST API's `Experiment` schema (`api-schema.yml:1086-1112`) has only 5 fields — `id` (UUID), `name`, `url`, `version_number`, `versions`. **No `pipeline_id`.** Worse, the `id` field is the UUID `public_id` (not the integer DB id), because `apps/api/views/experiments.py:36` sets `lookup_field = "public_id"`. **Fix shipped:** `PlaywrightBackend.pipelineIdFor` now scrapes from `/a/<team>/chatbots/<integer_id>/edit/`, which renders `SiteJS.pipeline.renderPipeline("#pipelineBuilder", "<team>", <pipeline_id>)` inline (`templates/pipelines/pipeline_builder.html`). Regex: `/renderPipeline\("#pipelineBuilder",\s*"[^"]+",\s*(\d+)\)/`. Live-verified 2026-04-09 against the sandbox team's source chatbot and multiple clones — every call returned a correct distinct pipeline id. The result is cached per experiment_id and populated eagerly by `cloneChatbot`.
12. **[LIKELY FINE, still unverified at scale]** **Rate limits on the OCS web routes.** Live-session 2026-04-09 made ~20 requests (form POSTs + page GETs + pipeline JSON + archive) in a few seconds without any throttling or WAF interference. No rate limits observed at small scale. Haven't tested at the volume of a real opportunity setup (~10-15 sequential calls) or with multiple opportunities racing. The `RestBackend` still implements exponential backoff on 5xx/429 for GETs; the Playwright backend serializes through the promise queue (added in the original implementation's CSRF fix) which bounds concurrency to 1.
...
15. **[RESOLVED, critical post-merge finding]** **`cloneChatbot` JSON-parse-of-redirect.** The initial implementation did `await copyRes.json()` to get `{experiment_id}`. `copy_chatbot` in `apps/chatbots/views.py:772` actually returns a 302 redirect to `single_chatbot_home`; there is no JSON body. **Fix shipped:** `RequestFn` options now include `followRedirects: false`; `cloneChatbot` disables redirect-following, reads the `Location` header, and parses the integer id from `/chatbots/(\d+)/`. Then scrapes `public_id` from the chatbot home page and `pipeline_id` from the edit page before creating the widget channel. Live-verified 2026-04-09 end-to-end.

16. **[RESOLVED, critical live-OCS finding]** **Form POSTs need `form:` encoding, not `data:`.** During live-OCS verification on 2026-04-09, a clone POST appeared to "succeed" with a 200 response — but the response body was the original chatbot's home page, not a redirect. `copy_chatbot` only issues the 302 redirect when `form.is_valid()` passes; otherwise it silently falls through to `return single_chatbot_home(...)`. The reason the form wasn't valid: Playwright's `data:` option sends JSON, but Django's `CopyChatbotForm(request.POST)` parses form-encoded data from `request.POST`. A JSON body with a `new_name` field doesn't populate `request.POST["new_name"]`, so the form thought `new_name` was missing. This issue affects every Django form POST in the backend: `/copy/`, `/channels/create-dialog/<platform>/`, `/documents/collection/new/`, `/versions/create`. **Fix shipped:** added `RequestOptions.formEncoded`, marked all form atoms with `formEncoded: true`, and updated the production request closure in `ocs-server.ts` to route between Playwright's `form:` / `multipart:` / `data:` options per flag. Live-verified end-to-end: clone actually returns 302 with Location, `createEmbeddedWidgetChannel` actually creates a channel, `setChatbotSystemPrompt` actually updates the prompt in the pipeline JSON.

17. **[RESOLVED, critical live-OCS finding]** **`extractPublicId` was matching the global OCS support widget.** On every authenticated page OCS renders a global `<open-chat-studio-widget chatbot-id="...">` tag for its own "Ask me!" help bot, with a fixed chatbot-id that is **not** the current experiment's public_id. The per-experiment widget tag only renders behind the `flag_chat_widget` feature flag — when the flag is off (default for the sandbox account), the only `chatbot-id` attribute on the page is the global support widget. The old regex was returning the same UUID for every experiment. **Fix shipped:** `extractPublicId` now anchors on the `#api-url-link` hidden input, which is always rendered and always specific to the current experiment: `<input id="api-url-link" value=".../api/openai/<public_id>/chat/completions" />`. Regex: `/id="api-url-link"[^>]*\/api\/openai\/([0-9a-f-]{36})\//`. Test fixtures updated to include both the api-url-link AND a global-support-widget-shaped decoy so regressions surface.
