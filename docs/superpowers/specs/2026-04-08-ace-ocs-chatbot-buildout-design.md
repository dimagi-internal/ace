...
7. **[RESOLVED for sandbox, PENDING for prod]** **Team slug.** For the test/dev environment, the team slug has been verified and recorded in the local `.env` (not committed). `.env.example` leaves it blank with a comment on how to find it. The production team slug — where real Connect opportunities will live — is still TBD and likely needs to be set up fresh (Dimagi doesn't currently have an ACE-owned team with the golden template on it).
