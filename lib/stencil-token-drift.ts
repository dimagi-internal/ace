...
export function formatStencilTokenDrift(report: StencilTokenDriftReport): string {
  ...
  lines.push(
    ...
    '',
    '  Pass no id: the script reads ACE_TRAINING_DECK_TEMPLATE_ID from',
    '  <plugin-data>/.env itself. Do NOT interpolate that variable in the shell —',
    '  ACE\'s env is loaded into MCP subprocesses, not the calling shell, so it',
    '  expands to EMPTY in a Bash tool call and would send an empty id (ace#1147).',
    '',
