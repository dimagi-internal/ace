...
This is the complete visual specification for Dimagi-branded external documents (work orders, MSAs, MOUs, SOWs, partner agreements). Every value below is exact. Helpers in `scripts/dimagi_styles.js` encode these — you should not need to set them by hand.

If you are writing styling code, use the helpers. If you are writing styling code in a language other than Node.js (e.g., python-docx), translate the values below into the equivalent calls in your library.
...
| Signature block label | 11pt | Deep Purple | Bold | Left | Inline labels like `Name:`, `Title:`, `Signature:`, `Date:` — see `scripts/dimagi_styles.js`. |
