...
3. **Grade across 9 dimensions.** Each dimension is 0–10. Overall
   ...
   | **Downstream handoff alignment** | 10% | (Added 0.13.84 — viability dimension; pairs with `idea-to-pdd-eval.demand_reality`.) The PDD named a downstream consumer at design time; at launch, re-verify they're aligned and ready to receive data. **Anchors:** downstream handoff confirmed with named delivery contract (data format, cadence, action-trigger threshold, recipient) = **9.5**; downstream confirmed at launch but contract is implicit = **7.5**; downstream named in PDD but no launch-time confirmation that they're ready = **6.0**; downstream missing from launch artifact OR vague ("analysts" generic) = **4.0**; firing launch into a downstream that hasn't materialized (PDD claimed Lab X; Lab X never engaged) = **2.0**. If `idea-to-pdd-eval.demand_reality` already scored ≤4 at design time, this dimension caps at 5 — you can't have launch-time alignment with a consumer that was never named. |
