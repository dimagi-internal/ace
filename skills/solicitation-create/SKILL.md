...
2. **Build the solicitation payload using the labs canonical schema.**
   **The labs MCP's `tools/list` is the canonical contract** — it
   ...
   | `expected_end_date` | string `YYYY-MM-DD` | `expected_start_date + the PDD's stated duration band` (take the top of the band when the PDD gives a range — e.g. `## Timeline` "~9–11 weeks" → start + 11 weeks). Must be strictly after `expected_start_date`. Use the Phase 4 opp `end_date` only when the Phase 4 dates were adopted wholesale under the rule above. **NOT** `anticipated_end`. |
