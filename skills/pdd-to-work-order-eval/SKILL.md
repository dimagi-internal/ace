...
- **Read declared parameters BY KEY, never by proximity (ace#2023).** Caps, rates, counts and reach live as named rows in the PDD's `program_parameters` block. Resolve `daily_cap_per_flw` (and any other parameter you cite) by its key; do NOT read an adjacent numeric row as the value. Measured on `poverty-graduation/20260905-1345`: this dimension struck the work order for omitting a `total_cap_per_flw` of 300 that does not exist — 300 is `expected_reach_min`, and the PDD states outright that no programme-wide per-FLW total cap is asserted. A strike resting on an invented parameter is worse than a missed one: it sends the producer to add a term the design never declared.
