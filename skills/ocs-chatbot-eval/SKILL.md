...
3. **Grade each response (LLM-as-Judge).** For each entry, score across 5
   ...
   | **Source usage** | 20% | Two parts: (a) **Structured citations** — the API `cited_files` field MUST be non-empty when `generate_citations: true` is set on the chatbot pipeline. **Two-tier cap (added 0.9.4):** empty `cited_files` + body text DOES name source docs by title = automatic ≤5 cap (the bot grounds, but the structured field is broken — pipeline bug); empty `cited_files` + body text also lacks named sources = automatic ≤3 cap (the bot is making it up; structural fail). (b) **Collection routing** — when populated, citations point to the correct collection (shared Connect vs. opp-specific) for the question's domain. |
