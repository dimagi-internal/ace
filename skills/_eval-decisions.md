...
1. **Binary correctness** — the artifact is "correct" or "wrong"; there's no spectrum to score. Examples: `app-deploy` (build succeeded or failed), `app-media-coverage` (attached or didn't), `app-connect-coverage` (clean or blocked).
...
| `app-hq-settings` | **NO eval** | Binary process — the two HQ-layer settings (camera-only `acquire` on Deliver uploads, per-module grid display) are applied or they aren't; the summary records `applied \| already-acquire \| not-required-by-pdd` states, nothing on a 0-10 spectrum. Released-build correctness is `app-release-qa`'s job; running-app quality is an `app-ux-eval`-shaped concern. Same rationale as `app-deploy` / `app-media-coverage`. |
...
| `app-media-coverage` | **NO eval** | Binary process — each attachment lands in the Nova blueprint or Nova rejects the batch and names the row. Nothing on a 0-10 spectrum. Whether the images are any GOOD is a real quality question, but it is a judgement about rendered screens, so it belongs to `app-ux-eval`, which already grades the FLW experience from captured screenshots — grading images here, before the app has ever been rendered, would be grading the plan rather than the result. |
...
| `app-release-qa` | **NO eval** | Structural + install-time QA — counts forms + checks Connect markers + greps XForm parse + runs commcare-cli validate + play. Binary outcome (`pass` / `fail` per mismatch class). Nothing on a 0-10 spectrum — every dimension is either structurally matched or it's a `[BLOCKER]`. The verdict-shape grades the deltas (form-count delta, missing marker count) but the rollup is binary. (Renamed from `app-release-smoke` 2026-05-27 for naming consistency with other `-qa` skills.) |
...
| `ocs-knowledge-refresh` | **NO eval** | Mechanical step — upload N documents, wait for indexing, republish. Binary success/fail with a named-miss list; there is no judgment to grade. The QUALITY of the documents it uploads is graded upstream by the five `training-*-eval` skills, and the quality of the resulting bot is graded downstream by `ocs-chatbot-eval --deep`. Grading the transport in between would measure nothing either of those misses. |
...
| `semantic-registry-author` | **has eval** | `semantic-registry-author-eval`. 5 dimensions: pdd_fidelity (0.30), definition_correctness (0.25), coverage_honesty (0.20), domain_expert_fitness (0.15, out-of-chain — would the programme's own M&E lead accept these definitions as theirs?), legibility (0.10). Provisional (ace#2510). |
| `demo-data-setup` | **NO eval** | Correctness here is binary and already fully covered by `demo-data-setup-qa` plus the live render: a dashboard either deep-links to a real run and paints real rows, or it does not. There is no 0-10 quality spectrum for a `${var}` map. The judgeable artifact of the phase is the NARRATIVE built on top of it, and canopy's concept and visual judges score that. |
| `demo-narrative` | **covered by sibling eval** | canopy owns the judges: `ddd-concept-eval` (concept clarity, design soundness, why-groundedness, claim-reality coherence, motion friction), `canopy:visual-judge` per scene, and `ddd-narrative-actionability-eval` (cold-derives a build plan from each scene's narration and scores it against the declared `features[]`). An ACE-side rubric would compete with canopy's and drift from the schema ACE does not own. |
...
| 2026-07-24 | Added `app-hq-settings` (**NO eval**) — Phase 3 Step 2.7 producer shipped without a registry row (caught by `test/lib/registries-coverage.test.ts`). Binary process per criterion 1: settings applied or not (`applied | already-acquire | not-required-by-pdd`), nothing on a 0-10 spectrum; released-build correctness belongs to `app-release-qa`, running-app quality to `app-ux-eval`-shaped concerns. | ACE team |
| 2026-08-27 | Replaced `app-multimedia-coverage` with `app-media-coverage` (**NO eval**, same rationale). Nova shipped a first-class media channel, so attaching is now a blueprint write rather than a post-release XML patch; the binary-correctness classification is unchanged, and image quality still belongs to `app-ux-eval`. | ACE team |
