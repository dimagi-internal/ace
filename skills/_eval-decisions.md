...
| `training-deck-outline` | **has eval** | `training-deck-outline-eval`. 5 dimensions: pedagogical_flow (0.30 — intro → reference → walkthrough → recap), screenshot_integration (0.25), anticipated_question_coverage (0.20), recap_effectiveness (0.15), section_weight_balance (0.10). Provisional. Verdict path renamed from self-emitted `training-deck-outline_verdict.yaml` to `training-deck-outline-eval_verdict.yaml` in `lib/artifact-manifest.ts`. |
