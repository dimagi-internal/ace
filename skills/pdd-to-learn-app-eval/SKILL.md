...
6. **Write the verdict YAML** to
   ...
   dimensions:
     ...
     language_conformance:      { weight: 0.08 }   # null + redistribute when PDD names no working language; English-only UI is CORRECT (full credit); HARD-FAIL on systematic inline language stacking or an in-app language selector (ace#968, inverted 2026-08-14)
   ```
...
2026-08-14 (ace#968):** the `localization_match ≤3 → fail` clause
(English-only against a French PDD) is REMOVED, not relaxed — English-only
is now the correct build, so the same artifact scores full credit on
`language_conformance` today and the 2026-05-29 `localization 2.0` figure
is history, not a target. `assessment_gating` and the depth cap still
force `fail` on their own, so the negative control holds. See
