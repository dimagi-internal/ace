...
| `app-deploy` | **inline QA** | Inline checks in `skills/app-deploy/SKILL.md`. § Step 2 — resolve `ACE_HQ_DOMAIN` (passed explicitly on every upload). § Step 2.5 — XML-escape lint walking every form, catches `<`/`>`/`&` in field labels that Nova's `validate_app` says ok but CCHQ build rejects. § Steps 3–4 — build-status check on each upload + `domain_not_authorized` BLOCKER if the HQ key can't reach `ACE_HQ_DOMAIN`. Class-level preventer per `docs/issues/nova-validate-app-misses-xml-escapes.md`. |
| `app-hq-settings` | **inline QA** | Inline checks in `skills/app-hq-settings/SKILL.md`. § Step 2 — form/module enumeration via run-form-walk against the DRAFT app uids, **halt if the walk reports `suite_xml`** (issue #108). § Step 3 — per-form acquire patch guarded by the `sha1` concurrency token: halt on `XformConflictError` surfacing the live sha1, `already-acquire` recorded when no `<upload>` needed changing (idempotent re-run). § Step 4 — per-module grid set, 200-confirmed, halt-loud on non-200; the UNVERIFIED app-root grid flag is deliberately not invented — recorded as an explicit `follow-up: app-root-menu-grid-unverified` line instead. § Step 5 — summary + residual resolution. Tight CCHQ HTTP iteration, same `inline QA` shape as `app-deploy`/`app-release`; `app-release-qa` (Phase 3 Step 2.8) is the independent released-CCZ backstop. |
...
| `app-media-coverage` | **inline QA** | Tight Nova iteration, and the structural half is already typed: `lib/media-plan.ts` validates every attachment before it is sent (per-target shape, slot legality, icon-tier membership, source/asset coherence), so a malformed row fails locally rather than costing a whole batch. What remains is the producer's own loop — plan, prepare, upload, attach, read the blueprint back. § Step 9 re-reads each touched form via `get_form` rather than trusting the call's return value. Released-CCZ backstop is `app-release-qa`. |
...
| `app-release-qa` | **IS the QA skill** | Structural + install-time QA on the released CCZ artifact. Inline checks in `skills/app-release-qa/SKILL.md § Step 4` walk the released-CCZ form XMLs vs. Nova blueprint, asserting form-count match + Connect-marker presence (`<learn:module>`, `<learn:assessment>`, `<learn:deliver>`, `<learn:task>`) + per-form field counts. § Step 4.5 runs `commcare-cli validate` + `play` as install-time runtime gates. Halt-loud on any structural or runtime mismatch. Renamed from `app-release-smoke` 2026-05-27 to match the producer/QA pairing convention used by the other QA skills (idea-to-pdd-qa, pdd-to-test-prompts-qa, etc.). |
...
| `ocs-knowledge-refresh` | **NO QA** | Its own product artifact IS the structural check: documents uploaded vs. documents expected, with the missing filenames named. A separate QA skill would re-derive the same count from the same source. The check that matters — `last_reindexed_at` present on a run whose Phase 6 completed — is asserted by `test/skills/ocs-knowledge-refresh.test.ts` and readable in the Phase 5 state file. **Revisit if:** the refresh grows a content transformation (e.g. chunking or summarising before upload) where what lands could differ from what was read. |
...
| Producer | QA status | QA skill / rationale |
|---|---|---|
| `semantic-registry-author` | **has QA** | `semantic-registry-author-qa` (ace#2510). The registry is parsed by labs' compiler and by every report bound to it, so structure gates real things: (1) live `semantic_registry_validate` — grammar, references, compiles at every scope; (2) `checkRegistryAuthoring` (`lib/semantic-registry-authoring.ts`) — model declared, every indicator has a numerator/denominator measure, a PDD section citation that resolves, no target the PDD did not state, % bands not written as fractions, unique headline positions, the programme's own nouns in `display:`, every partner opportunity in `llo_map`. Static; no LLM. |
| `demo-data-setup` | **has QA** | `demo-data-setup-qa`. 6 static checks on the handoff, run BEFORE `demo-narrative` authors scenes against it: `realized.json` parses and is FLAT (DDD substitutes `${var}` verbatim, so a nested value films a literal placeholder), every `<key>_par_url` matches the run deep-link shape (the two verified bounce traps — no `run_id` renders the workflow LIST, no `/run/` renders the DEFINITION page — are silent, and a blank dashboard passes every create call), planned dashboards match the realized map, the opp is labs-only (id >= 10000), the timeline is a pinned Monday, deliver units captured. Structural only; no LLM. |
| `demo-narrative` | **NO QA** | Gated by canopy, not by ACE. `scripts.ddd.validate` (schema + semantic rules on both WhyBrief and UnifiedSpec), `spec_qa` (concept-claim falsifiability, show-don't-tell action fidelity) and `narrative_coherence` (outcome leakage) all run against the authored artifacts and must pass before handoff. ACE does not own the narrative schema, so an ACE-side QA skill would duplicate canopy's boundary and drift from it. |
...
| 2026-07-24 | Added `app-hq-settings` (**inline QA**) — the Phase 3 Step 2.7 producer shipped without a registry row (caught by `test/lib/registries-coverage.test.ts`). Classified per the existing inline-QA heuristic: tight CCHQ HTTP iteration (run-form-walk enumeration with a `suite_xml` halt, sha1-guarded acquire patches with `XformConflictError` handling, 200-confirmed per-module grid sets), with `app-release-qa` as the independent released-CCZ backstop. | ACE team |
| 2026-08-27 | Replaced `app-multimedia-coverage` with `app-media-coverage` (**inline QA**). The old skill's QA burden was the fragile half of its pipeline — form-XML patching, CCZ bundling order, CCHQ orphan-pruning — all of which Nova's native media channel retires. The replacement's structural checks move into a Zod schema (`lib/media-plan.ts`) that fails a bad attachment locally, which is why this stays inline rather than becoming a `-qa` skill. | ACE team |
