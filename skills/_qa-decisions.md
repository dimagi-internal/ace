...
| `app-test-cases` | **NO QA** | Mobile MCP atom `mobile_validate_recipe` validates each generated recipe at boundary. Producer dispatches the validator per recipe and acts on its verdict (re-resolve form/field IDs from Nova on failure). The producer's iteration loop wraps the MCP validator — the validation IS the MCP, ACE doesn't add structural QA on top. If `mobile_validate_recipe` doesn't catch a class of bad recipes, that's an MCP improvement, not an ACE-side QA addition. Plus a recipe-wide `mobile_resolve_selectors` gate (added 2026-05-12, Step 3.4) that halts authoring on selector-map gaps before recipes are written to Drive — shifts Phase 5 blockers to Phase 2 where Nova context is in-scope. Same MCP-boundary heuristic: validation IS the MCP. |
...
| `training-deck-outline` | **inline QA** | Process step 4 self-checks the in-memory draft inline before `drive_create_file`: single `# Title` heading, ≥4 `## Slide:` headings, every image ref is `drive:<fileId>` or `https://...`, every speaker-notes block non-empty. Downstream consumer `training-deck-build` re-parses the same file via `parseDeckOutline` (`lib/training-deck-spec.ts`) at its Step 2; producer's inline check approximates the consumer's structural contract — extracting a separate `-qa` skill would duplicate Step 4's checks for no additional coverage. |
