...
5. **Upload RAG files (PDD + source inputs + training + summaries).**
   ...
   **ground truth**. Indexing it makes the pipeline "plant the answer key
   ...
   - `00-program-contacts.md` — **generated by this step**, not read from
     Drive (composition rule below). The only file in this recipe that
     carries the program's escalation contacts.
   ...
   **Compose `00-program-contacts.md` — the KB's only contacts anchor
   (dimagi-internal/ace#1665).** Before this file existed, every address the
   bot hands out lived ONLY in the composed system prompt, with no retrievable
   source anywhere in the collection — so the value was reproduced from prompt
   recall alone and retrieval could not correct a drift. Measured on
   `hh-poverty-targeting/20260824-1404` (experiment 13014, collection 566, 11
   files, 73-prompt deep run): the bot emitted `ace@dimagi.com` twice and
   invented `pm@dimagi-ai.com` once, against 27 correct emissions; the only
   email address in the whole indexed KB was a reviewer's. The golden
   template's prompt already names the exact address *and* forbids the
   misspelling by name (`scripts/bootstrap-ocs-golden-template.ts`), which is
   what makes this a **KB-recipe** defect rather than a prompt defect — a
   prompt-side guard cannot anchor a value that has no retrievable source.
   (Distinct from ace#1142, whose invented-emergency-number half did not
   recur on this run.) Under `ocs-chatbot-eval` § `fabricated_operational_specifics`
   a contact email is an operational specific, so each drifted entry clamps to
   Fail.

   Every rule for composing it is about NOT inventing:

   - **Read the ACE escalation address; never type it from memory.** It is
     `.email` in `config/agent.json` (`ace@dimagi-ai.com`) — the same value
     `bin/ace-email` sends as and the golden template names. Emit it verbatim.
   - **Per-opp roles come from the run's own state, not from you.** Network
     Manager, LLO contact, operator: take each from this run's
     `run_state.yaml` (`phases.*.products.*`) or the PDD. Emit only what a
     source actually supplies.
   - **When a role has no published contact, SAY SO — do not supply a
     plausible one.** Write the role with an explicit *"no contact published
     for this opportunity; escalate to the ACE admin group"* line. A
     fabricated-but-plausible address is the exact defect this file exists to
     fix, and an invented `pm@dimagi-ai.com` is worse than a typo because it
     reads as authoritative.
   - **Contacts only — no scoring or instrument content.** This file must
     carry no point values, no lookup-table rows, no thresholds, and no
     expected answers. It is a small anchor and must not become a route for
     content the collection deliberately excludes.

   Shape (kept deliberately small, so a retrieval hit returns the whole of it):

   ```markdown
   # Program contacts — <opp-name>

   ## Escalation
   - ACE admin group: ace@dimagi-ai.com
     This is the only correct spelling. Not `ace@dimagi.com`, and not any
     other local part at `dimagi-ai.com`.

   ## Roles
   - Network Manager: <value from run state / PDD, or: no contact published
     for this opportunity; escalate to the ACE admin group>
   - LLO contact: <same rule>
   ```
...
7. **Compose the system prompt** from the PDD + opp details + escalation rules. The prompt MUST:
   ...
   - **Tell the bot to escalate on specific triggers, and point it at the
     `00-program-contacts.md` file in its collection for the address rather
     than carrying a second copy of it inline** (dimagi-internal/ace#1665).
     That file is the retrievable anchor; a prompt that restates the address
     is a duplicate that can drift independently of the KB, which is the
     configuration that produced `ace@dimagi.com` / `pm@dimagi-ai.com` on a
     73-prompt deep run. Where the prompt does name the address, it must be
     the value read from `config/agent.json` `.email` — never typed from
     memory — and must say the contacts file is authoritative.
...
| 2026-08-26 | **Step 5: the KB recipe now generates and indexes `00-program-contacts.md`; Step 7 points the prompt at it instead of restating the address (dimagi-internal/ace#1665).** The recipe (PDD + inputs + training + app summaries) carried no file with any contact in it, so the escalation address existed only in the composed prompt — reproduced from recall, with nothing retrievable to correct a drift. On `hh-poverty-targeting/20260824-1404` (experiment 13014, collection 566) a 73-prompt deep run emitted `ace@dimagi.com` twice and invented `pm@dimagi-ai.com` once against 27 correct emissions; the only address in the whole indexed KB was a reviewer's. A prompt-side guard cannot anchor an unsourced value — the golden template already names the exact address and forbids the misspelling by name, and it drifted anyway. The anchor is deliberately structural: the ACE address is READ from `config/agent.json` `.email`, per-opp roles come from the run's own state, and a role with no published contact is written as such rather than filled with a plausible-looking address. Contacts only — no point values, lookup rows, or thresholds may ride in on it. *Enforced:* `test/skills/kb-contacts-anchor.test.ts`. | ACE team |
