...
|---|---|---|
| `Failed to start learning` | **HISTORICAL ROW — re-interpret.** Original hypothesis (pre-2026-05-22): the Nova `<module xmlns="…connect…">` + `<assessment xmlns="…connect…">` wrappers in the released Learn CCZ cause the AVD's CommCare runtime to fail at Learn-launch. nova-plugin#7 closed 2026-05-22 with the maintainer's account that wrappers are **required** for Connect's HQ→Connect sync (registers learn modules + deliver units), and the actual root cause of every wrapper-attributed failure we saw was Connect block IDs exceeding the server-side 50-char limit, fixed at the Nova emitter by commcare-nova PR #21. If you hit this error today: DO NOT run `commcare-form-patch` (quarantined). Verify Connect block slugs are ≤50 chars (`/ace:doctor` + Connect-API probe). If slugs look fine and the AVD still chokes, the original hypothesis may not be fully retired — open an issue with adb logcat + a UI dump from the failure screen. Diagnostic probe: `npx tsx scripts/probe-connect-learn-handoff.ts <opp_uuid>`. |
