...
- **Form fields, per walked form** — `nova_get_form({app_id,
  ...
  **screen-shape aware**: it turns on the `label`-screen carve-out
  ...
  leading-label check (#1045), and the score-gated over-advance check
  (#1118). Omit it and all degrade to the old field-blind behaviour —
  which false-positives on any label-heavy Learn app.
...
| `option-tap-guard-is-unscoped` | A **global** `when: notVisible: "<option>"` guard (and its scroll) in front of a **`below:`-scoped** option tap — the option-tap sibling of `input-focus-scroll-is-guarded`. A `when:` condition owns its own element selector and `below` strictly narrows the candidate set, so the guard asks *"is there no `No` anywhere"* while the tap needs *"a `No` below MY question"*. On a field-list of identically-labelled Yes/No questions a bare option is essentially always on screen, the guard is a runtime **no-op**, the scroll never fires, and the tap dies as soon as its own options fall below the fold (`poverty-graduation/20260915-1518`: `Element not found: Text matching regex: No, Below: ...MILK...`; scoping all 13 pairs took the leg to `pass`). Give the guard AND its `scrollUntilVisible` the tap's own `below:` anchor — scope **parity**, not "always scope": a bare tap under a global guard is still correct (ace#1070). Re-author via `/ace:step app-test-cases` (ace#2426). Needs no field data — pure recipe shape. |
| `input-without-erase` | An `inputText` step with no `eraseText` immediately before it. Maestro's `inputText` **appends at the cursor** — it does not replace — so a field already holding a Nova casedb preload, an XForm default, or a stray character receives the two values concatenated. Live: `spark-facilitator/20260828-0703` typed `40` into `hh_represented_at_the_meeting`, submitted `140`, and the form's cross-field constraint refused to advance; the leg then died two screens later on an unrelated scroll, reading as a selector fault it was not. Wherever it does **not** trip a constraint the corruption is **silent** and the leg reports `pass` on wrong data. Emit `- eraseText` before every `- inputText` (a redundant erase on an empty field is a runtime no-op); re-author via `/ace:step app-test-cases <opp>/<run-id>` (ace#1844). Needs no field data — the defect is pure recipe shape. |
...
[#618](https://github.com/jjackson/ace/issues/618) landing, now **HEALED in
...
**Recognized failure modes** (surface a `[BLOCKER]` in `auto_surfaced`
...
node "$ACE_ROOT/node_modules/tsx/dist/cli.mjs" "$ACE_ROOT/scripts/probe-atlas-drift.ts" <screenshotDir> \
