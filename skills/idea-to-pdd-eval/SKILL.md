...
4. **Grade across 10 dimensions.** Each dimension is 0–10. Overall
   score is the weighted mean.

   Five dimensions are **document quality + fidelity** (50% weight); five are **program viability** (50% weight). The viability axis was added in 0.13.81 in response to the holistic-probe finding that the rubric was scoring 8.65/10 on PDDs an adversarial reader gave 3/10 viability. In 0.13.84, `stress_test_agreement` was removed (tautological — same model + same rubric applied twice) and weights were rebalanced to a clean 50/50 split. (The original 0.13.81 weights summed to 0.95, not 1.0 — bug fixed in 0.13.84.)
...
5. **Write the verdict YAML** to
   ...
   dimensions:
     ...
     # Program viability (50%, expanded 0.13.84)
     demand_reality:               { weight: 0.20 }
     resource_realism:             { weight: 0.15 }
     mission_alignment:            { weight: 0.10 }
