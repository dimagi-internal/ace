...
5. **Grade across 10 dimensions** — 5 conformance (45%) + 5 fitness
   ...
   *Fitness axis (55% — is it a deployable instrument, graded vs expert bar):*
   ...
   | **Data-quality validation** | 13% | Does the instrument *enforce* data quality? Graded vs what a deployable form should constrain, NOT vs the PDD: numeric bounds on counts (`household_size 1–30`), cross-field checks (`under_5 ≤ household_size`), **two-sided bounds on every date field**, phone-format regex where a phone field exists, char limits on free text, required `validate` on every credit-bearing field. **Dates are graded like counts, not like the PDD (ace#1788).** A date bounded on only ONE side is an unbounded field, and the PDD specifying only one side is not a defence — this is a fitness dimension. Check each date field for a bound on BOTH sides: an upper (typically `. <= today()` for a past event) and a plausible bound on the open side derived from the programme's own window (enrolment period, visit cadence, recall horizon) and disclosed in the build memo. Do NOT require a specific window — the floor is programme-specific; require that one exists and is justified. **Escalation:** where a downstream programme rule does arithmetic on the field (a minimum gap between visits, an age computation, an eligibility window), a one-sided bound is a **2-point deduction**, not 1.5, and carries a `[WARN]` naming the rule it defeats — the failure is not merely bad data but a check that reports green on it. Live anchor: `bednet-check-2-visit/20260828-0629`, `bednet_given_date` with `. <= today()` and no lower bound, on a programme whose R7 requires ≥3 days between registration and follow-up; a registration typed `2025-08-14` instead of `2026-08-14` satisfies the field constraint AND the gap check. That build scored 8.5 on this dimension. **Hard-gate:** a data-capture instrument with near-zero validation (only a consent check + one range) → dimension **≤3**. Each whole class of missing constraint that a deployable build needs (counts unbounded, phone unformatted, free-text uncapped, dates one-sided) = 1.5-point deduction. |
   ...
   - **`option_register_fidelity`** (added 2026-08-24, ace#1621) — **when the
     ...
     **Do not eyeball this and do not re-derive it — run
     ...
     - The build memo's `option_register:` block present, finding count `0`,
       register tag echoed, **and `verified: true`** → no finding. A triggered
       field with NO such block and no recorded skip reason → `[BLOCKER]` →
       `fail`: the check not having run is indistinguishable at this layer from
       it having been skipped because it would have failed.
     - **`verified` missing or not `true` → `[BLOCKER]` → `fail` (ace#1886).**
       Since 2026-09-06 Step 4f binds the register itself rather than handing
       the bind to an operator, so the build now makes a claim it did not make
       before — *this select draws from the partner's table* — and `verified`
       is the only evidence for it. It must come from `verifyLookupBind` over a
       `get_field` read-back: Nova answers a *correctly bound* lookup field with
       `"options": []` and no mention of the source, so the write's own response
       cannot confirm a bind, and a build that treats "no error" as success is
       asserting something it never observed. An unbound select renders **empty**
...
2026-08-14 (ace#968), then RE-amended 2026-08-17 (PR #1463, superseding ace#968/#1391):**
