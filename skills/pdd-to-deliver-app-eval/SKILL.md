...
5. **Grade across 9 dimensions** — 5 conformance (45%) + 4 fitness
   ...
   *Fitness axis (55% — is it a deployable instrument, graded vs expert bar):*
   ...
   | **Language conformance** | 8% | **HARD-FAIL dimension — inverted 2026-08-14 (ace#1391, superseding ace#968).** Applies only when the PDD names a working language other than English; otherwise `null` + redistribute. **ACE builds every app UI in ENGLISH ONLY** (standing decision, Jon, 2026-08-14 — see `_app-component-library.md § english-only-ui`). The working language is context for training, facilitation and the OCS chatbot; it is NOT a delivered app property. Grade the build against that: (a) **every user-facing string English-only, no translated counterpart anywhere** → full credit, plus an `[INFO]` recording that the PDD named <LANGUAGE> and the app correctly shipped English-only; (b) **a few stray stacked or parenthetical translations** (a handful of labels carrying `English / <LANGUAGE>: …`, a translated option set) → **5** + `[WARN]` naming each string, since the retired mechanism is still leaking into builds; (c) **systematic inline stacking** (the app is broadly authored with every label carrying N languages, the pre-2026-08-14 pattern) → **≤3 → suite `fail`**; (d) a **language-selector question** or any in-app language switch → **≤3 → suite `fail`** (it promises a capability the build cannot deliver). **Do NOT deduct for the absence of translations — that is now the correct build.** The prior rubric hard-failed the mirror image (English-only was the failure, complete inline coverage was full credit); a judge carrying the old rule inverts every verdict on this dimension, so read the criteria, not your memory of them. **Quality note, not a deduction:** since the English IS the interface for a non-native reader, surface a `[WARN]` where stems or labels are long, idiomatic or clause-heavy. |
...
6. **Write the verdict YAML** to
   ...
   dimensions:
     ...
     language_conformance:     { weight: 0.08 }   # null + redistribute when PDD names no working language; HARD-FAIL on English-only or incomplete coverage (inline coverage = the sanctioned mechanism, full credit)
   ```
...
8. **Auto-surfaced concerns** (per `_eval-template.md § Auto-surfaced
   ...
   - `[INFO]` when the PDD names a working language other than English and
     the app correctly shipped an English-only UI — recorded so a reader can
     see the absence of translations was the standing decision, not an
     omission (ace#968).
...
2026-08-14 (ace#968):** this anchor also required `localization_match ≤3
→ fail` (English-only against a French PDD). That clause is REMOVED, not
relaxed — English-only is now the correct build, so the same artifact
scores full credit on `language_conformance` today. The three dimensions
above still force `fail` on their own, so the negative control holds; a
judge that re-derives the retired clause is mis-calibrated in the
direction this rubric just reversed. See
`docs/superpowers/specs/2026-05-29-eval-fitness-gap.md`.
