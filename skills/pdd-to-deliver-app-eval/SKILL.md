...
4. **Grade across 5 dimensions.** Each dimension is 0–10. Overall
   ...
   | **Connectify wiring** | 25% | Deliver Unit name matches PDD spec exactly. Entity ID composite formula matches the PDD's stated formula (or, where the PDD doesn't fully specify, is sensible — e.g. includes market_name + GPS hash for atomic-visit). Required-for-credit fields (typically photo + GPS + consent) are wired correctly. |
