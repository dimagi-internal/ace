...
4. **Create the program** via `connect_create_program`:
   ...
   - `currency` / `country`: **read `opportunity_currency` and
     ...
     The GATE works in ISO 3166-1 alpha-3 (the helper normalizes alpha-2
     and plain names, so pass through whatever the PDD wrote), but **SEND
     `country: connectCountryName(verdict.country)`, never the alpha-3.**
     Connect's create serializer matches `country` on `Country.name`, so
     alpha-3 is rejected — live 2026-09-26, spark-facilitator/20260925-1536
     Phase 4: `country: 'MWI'` → *"country: Object with name=MWI does not
     exist."*; `country: 'Malawi'` → created, and reads back as `MWI`
     (ace#2486). The write takes the name and the read returns alpha-3, so
     do not "fix" the send value to match the read-back. HALT if
     `connectCountryName` returns `null`; never type a spelling by hand.
     Send `currency` as the ISO 4217 code (`verdict.expected`).
...
| 2026-09-26 | **Step 4 SENDS the country NAME, not the alpha-3 (ace#2486).** The 2026-09-16 row told Step 4 to send the normalized `verdict.country` (alpha-3), regressing the correct 2026-04-30 contract: Connect's create serializer is `SlugRelatedField(slug_field="name")` on `Country`, so `country: 'MWI'` failed live with *"country: Object with name=MWI does not exist."* (spark-facilitator/20260925-1536 Phase 4, 2026-09-26) while `'Malawi'` succeeded and read back as `MWI`. The gate still works in alpha-3; the send value is now derived via `connectCountryName` (`lib/program-locale.ts`), whose `CONNECT_COUNTRY_NAME` is copied from Connect's own `COUNTRIES` seed (migration `0092`), not a generic ISO list. *Enforced:* `test/lib/program-locale.test.ts` (coverage of every `COUNTRY_CURRENCY` key, MWI → Malawi, unknown → null, name round-trips through `normalizeCountry`). | ACE team |
