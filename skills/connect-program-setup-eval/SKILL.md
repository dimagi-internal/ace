...
5. **Grade across 6 dimensions.** Each dimension is 0–10. Overall
   ...
   | **Payment-unit fit** | 14% | Payment structure must match the PDD's intent: per-delivery for atomic-visit, **one unit for the whole arc for longitudinal-visits** (sequence in `entity_id`, registration unpaid — NOT one unit per phase), per-session for focus-group, per-stage for multi-stage. Mismatch is a 3-point deduction. **A `longitudinal-visits` opp has none of the other three payment shapes and must not be read as a mismatch against them** — that is ace#2294: this row named only the other three, so on `bednet-check-2-visit/20260908-1544` a correctly configured opportunity was gradeable as a 3-point deduction on a 14%-weighted dimension, and only survived because the judge noticed the gap and reasoned around it. For that archetype also check the **`entity_id` grain** — the composite must carry entity + sequence position, and a present-but-UNCLAMPED sequence component is byte-identical to a clamped one, so the record must state the clamp semantics per component rather than merely listing them (ace#1462, ace#1810; the three failure modes are enumerated in `connect-opp-setup § Archetypes`). (Rate *affordability* against a real-world benchmark is graded in **Deployability**, not here — this dimension is structural-fit only.) |
...
| `longitudinal-visits` | Grades **one** payment unit for the whole arc — not one per phase — with the sequence expressed in `entity_id` and registration unpaid, plus the longitudinal clause of Layer A. Adds an **`entity_id`-grain sub-check** under `payment_unit_fit`: the composite must resolve to entity + sequence position, against the three failure modes `connect-opp-setup § Archetypes` enumerates — (1) the case id **alone** folds the whole arc onto one entity row; (2) a `concat(username, today())`-style key reverts to cross-sectional grouping; (3) **the sequence component is present but UNCLAMPED**, so the PDD's per-entity grouping silently does not hold. None of the three stops a payment — a repeated key is paid again (ace#2512) — so the per-entity cap is graded as enforced only with a saved `payable_slot` `form_field_rules` row. Mode (3) produces a byte-identical composite, so a record that lists the components is not evidence — it must state, per component, whether the clamp is implemented (ace#1462, ace#1810). |
