...
| `longitudinal-visits` | Grades **one** payment unit for the whole arc — not one per phase — with the sequence expressed in `entity_id` and registration unpaid, plus the longitudinal clause of Layer A. Adds an **`entity_id`-grain sub-check** under `payment_unit_fit`: the composite must resolve to entity + sequence position, against the three failure modes `connect-opp-setup § Archetypes` enumerates — (1) the case id **alone** pays each entity once, ever; (2) a `concat(username, today())`-style key reverts to cross-sectional dedup and makes repeat activities payable; (3) **the sequence component is present but UNCLAMPED**, so the PDD's per-entity cap silently does not bind. Mode (3) produces a byte-identical composite, so a record that lists the components is not evidence — it must state, per component, whether the clamp is implemented (ace#1462, ace#1810). |
