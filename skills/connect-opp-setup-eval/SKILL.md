...
5. **Grade across 6 dimensions.** Each dimension is 0–10. Overall
   ...
   | **Payment-unit fit** | 0.16 | Payment-unit *count* must match what the PDD declares (one per-delivery PU for atomic-visit; **one PU for the WHOLE ARC for longitudinal-visits** — the sequence lives in `entity_id`, not in the PU count, and registration is unpaid; one per-session for focus-group; one PU per stage for multi-stage). Payment *type* (visit/session/stage/piece) must match. Mismatch in count = 3-point deduction; mismatch in type = 3-point deduction. **Do not read a `longitudinal-visits` opp against the other three shapes** — a single PU there is the CORRECT structure, not a missing per-phase unit (ace#2294: this rubric named only the other three, so a correct configuration read as a mismatch against all of them and drew the deduction). (Rate *affordability* against a real-world benchmark is graded in **Deployability**, not here — this dimension is structural-fit only.) |
...
| `longitudinal-visits` | Grades **one** payment unit for the whole arc (NOT one per phase) with the sequence carried in `entity_id` and registration unpaid, plus the longitudinal clause of Layer A. Adds an **`entity_id`-grain sub-check** under `archetype_config_coherence`: the composite must resolve to entity + sequence position, and the grader checks the three failure modes `connect-opp-setup § Archetypes` enumerates — (1) the case id **alone**, which folds the whole arc onto one entity row; (2) a `concat(username, today())`-style key, which silently reverts to cross-sectional grouping; (3) **a sequence component that is present but UNCLAMPED**, which produces a byte-identical composite and puts every visit past the PDD's per-entity cap on its own key. **None of the three stops a payment** — Connect pays a repeated key again with the `duplicate` flag off (ace#2512) — so a PDD per-entity cap is graded as enforced ONLY when a `form_field_rules` row on the Deliver form's `payable_slot` field was saved; a record that cites the key alone as the cap's enforcement is a hard deduction. Mode (3) is invisible to a reader of the composite alone, so a record that merely LISTS the components is not evidence — the record must state, per component, whether the PDD's sequence/clamp semantics are implemented (ace#1462, ace#1810). |
