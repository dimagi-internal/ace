...
1. **Workflow wiring (weight 0.30).** Both workflow IDs present in the
   summary AND in `outputs.synthetic.workflows` (current run's
   `run_state.yaml`; legacy fallback `opp.yaml.synthetic.workflows`).
   The audit's
...
- `[BLOCKER]` if `outputs.synthetic.workflows` (and legacy `opp.yaml.synthetic.workflows`) was not updated.
