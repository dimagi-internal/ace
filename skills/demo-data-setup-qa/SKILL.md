...
| 2 | `every_par_url_shape_valid` | static | `primary_par_url` is present, and every `<key>_par_url` is a valid labs workflow URL **for its dashboard's `shape`**: **run** → matches `^https://labs\.connect\.dimagi\.com/labs/workflow/\d+/run/\?run_id=[^&]+&opportunity_id=\d+$` (saved-run deep-link, NOT the bare picker); **action** → matches `^https://labs\.connect\.dimagi\.com/labs/workflow/\d+/\?opportunity_id=\d+` (view URL, no `run_id`) | run-shaped: rebuild from the saved `run_id` (learning 2026-06-13). action-shaped: use the workflow view URL — it has no saved run |
