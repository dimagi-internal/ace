...
| 2 | `every_par_url_is_run_deeplink` | static | `primary_par_url` is present, and every `<key>_par_url` matches `^https://labs\.connect\.dimagi\.com/labs/workflow/\d+/run/\?run_id=\d+&opportunity_id=\d+$` — **BOTH run- and action-shaped dashboards render at this deep-link** (action-shaped's `run_id` is minted via `workflow_create_run`). | point at a real `run_id` (mint one for action-shaped). Verified-live traps: `/run/?opportunity_id=` with no `run_id` → workflow LIST; `/workflow/<def>/?opportunity_id=` (no `/run/`) → DEFINITION page (2026-07-21) |
