...
| **Screenshot integration** | 0.20 | **Seed this dimension STRUCTURALLY before judging: read `visual_coverage.ratio` from the generate verdict (or recompute via `computeVisualCoverage`, `lib/training-deck-spec.ts`). It is opp-specific coverage only — pool and template images are excluded by construction (ace#856/#873), so do not credit them.** Then judge grounding quality on top. 10 = `ratio` ≥ 0.9, every Walkthrough step names a resolving screenshot. 6 = `ratio` 0.5–0.9; main flow grounded, edges thin. 3 = `ratio` < 0.5 OR ≥ 1 dead Drive-ID reference. Hard-deduct -3 per dead Drive-ID reference. **Sampling is MANDATORY and MUST be stratified by stencil type, including at minimum 3 slides that SHOULD carry per-opp screenshots (`your-opportunity` / `practice` walkthroughs).** An unstratified spot-check samples the image-bearing slides — the only population that cannot fail — and is not evidence for any score. |
...
- **Opp visual coverage < 0.2 → cap overall ≤ 4.0 and set suite verdict `fail`, regardless of how good the prose is** (ace#856). This is arithmetic, not judgment: a deck whose walkthrough spine has no screens is a placeholder, and the prose being excellent is precisely how it passed before. The measured case: 9% image coverage scored **9.5 pass** here and was emailed to the operator, who opened it and said "almost nothing in it". A 9.5 on that deck must be structurally impossible, not merely unlikely.
