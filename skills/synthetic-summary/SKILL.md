...
2. **Compose the summary** at
   ...
   **Fixture folder (read-only, for review):** <GDrive folder URL from synthetic-data-generate.md>
   ...
   - **LLO Weekly Review** — when `synthetic-workflow-seed.md` recorded a saved run, deep-link the latest run so the polished dashboard renders directly (skips the run picker / "select an opportunity" gate): `${LABS_BASE_URL}/labs/workflow/<llo_weekly_review_id>/run/<week_2_run_id, else week_1_run_id>/?opportunity_id=<labs_opp_id>` (clickable). If no run was saved, fall back to `${LABS_BASE_URL}/labs/workflow/<llo_weekly_review_id>/?opportunity_id=<labs_opp_id>` (lands on the run picker).
   ...
   - **Program Admin Audit** — deep-link the saved audit run when recorded: `${LABS_BASE_URL}/labs/workflow/<program_admin_audit_id>/run/<audit_run_id>/?opportunity_id=<labs_opp_id>` (clickable); else `${LABS_BASE_URL}/labs/workflow/<program_admin_audit_id>/?opportunity_id=<labs_opp_id>`.
...
| 2026-06-13 | Demonstrative-workflow links now deep-link the saved run (`/labs/workflow/<id>/run/<run_id>/?opportunity_id=<opp>`) so they render the polished dashboard directly instead of the run picker; fall back to the bare URL only when no run was saved. Recipe per `docs/learnings/2026-06-13-labs-workflow-run-deeplink.md` (jjackson/ace#769). | ACE team |
