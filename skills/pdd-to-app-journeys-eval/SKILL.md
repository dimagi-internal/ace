...
This skill ships **provisional** until calibrated against ground truth. Initial anchors below; refine per `eval-calibration` methodology.
   ...
   | **Deployability fitness** (out-of-chain) | 25% | **Anchor is OUTSIDE the AI authoring chain** — not the PDD. The question: *would a CommCare field expert ship an app built from this journey set into real data collection?* A deployable instrument demands things the journeys must drive even when the PDD never mentioned them. Check whether the journeys demand: (a) **input validation** — required-field enforcement, type/range constraints, implausible-value rejection (a date-of-birth in the future, a negative count); (b) **GPS capture fidelity — observability, NOT a gate** — where the intervention is location-bound, journeys must demand more than "capture location": the fix's own accuracy read back to the FLW on screen, a retry affordance on a poor fix, a stated target tolerance, and `gps_accuracy_m` submitted with EVERY outcome so review can weight on it. An advisory whose branches cover only a band *below* the tolerance — silent on the worst readings — is the material gap here. **Do NOT demand, and do NOT deduct for the absence of, a hard gate that rejects or blocks submission on a low-accuracy fix.** That gate is a Table-A known-unbuildable mechanism (`skills/_app-component-library.md` § Mechanisms a PDD must not assert): Nova rejects `validate` on `kind: geopoint` (#695/#699, as recorded in that section), the adjacent-question workaround hard-fails `app-release-qa`'s constraint-locality check (ace#723, PR #988), and Connect's verification-flags form no longer renders `gps` / `gps_radius_meters` — `connect_set_verification_flags` REFUSES them with `unsupported_verification_flag` (ace#1006, ace#1013). A journey set that demands an advisory-with-retry **rather than** a block is scoring at the TOP of this anchor, not the bottom; and where the PDD deliberately declines to fix a radius, journeys that name that as a residual instead of inventing a threshold are the CORRECT behaviour, not a gap. `idea-to-pdd` § Step 4a and `pdd-to-deliver-app-eval` § Capture fitness were corrected to this contract on 2026-07-28; this rubric is the half that sweep missed (ace#1619); (c) **case write-back on follow-up** — multi-visit / longitudinal archetypes must update the existing case, not silently create duplicates; (d) **data-quality enforcement** — photo legibility, duplicate-household detection, consistency checks across fields. Two symmetrical cautions here, and BOTH were checked for ace#1619. Connect's retired `duplicate` **verification flag** (ace#1013) is one surface among several, so its absence does NOT by itself make duplicate handling unbuildable — that inference was made and refuted. But do not swing the other way either: this rubric does not assert which duplicate mechanism is available for the archetype in hand, so apply the Table-A/Table-B check below to (d) exactly as to (b). Where the journeys state, WITH A REASON, that no duplicate control exists on the surfaces this design can reach, that is a residual to surface (`[INFO]`) — not a deduction. **This dimension is EXEMPT from any deferral carve-out:** a PDD silence on validation/GPS/write-back is exactly the gap to surface, not a reason to skip the check. **Anchors:** journeys demand all relevant deployability behaviors a field expert would require, with concrete recovery paths = **9.0–10**; demands most but ≥1 material gap (e.g. captures GPS but never shows the fix's accuracy to the FLW and never submits it with the record; an accuracy advisory with no above-tolerance branch) = **6.0–7.0**; happy-path-only journeys that move data through screens but demand no validation, no write-back, no quality enforcement = **≤3.0**; thin happy path with the PDD silent and the journeys inheriting that silence wholesale = **≤3.0**. **Before deducting on ANY of (a)–(d), check the behaviour you are about to require against `_app-component-library.md` § Mechanisms a PDD must not assert.** A Table-A mechanism is closed at the platform surface, so its absence is not a gap and demanding it is the ace#1238 guard-predicts-a-capability failure inverted — a rubric grading against something no build can produce, on every opportunity, unrecoverably. A Table-B mechanism is buildable but unsupported this cycle: name it as a residual (`[INFO]`), do not deduct. **Hard gate:** if the journey set describes only a thin happy path with none of (a)–(d), score ≤3 and the suite verdict is `fail` regardless of the other six dimensions. A faithful build of a thin skeleton is undeployable — that is the failure this axis exists to catch. |
...
| Date | Change | Author |
|------|--------|--------|
| 2026-09-06 | **`deployability_fitness` anchor (b) stops demanding a GPS accuracy gate ACE cannot build (closes dimagi-internal/ace#1619).** The anchor required journeys to "reject low-accuracy fixes" and named "captures GPS but never gates on accuracy" as the deduction exemplar. That gate is a **Table A** known-unbuildable mechanism: Nova rejects `validate` on `kind: geopoint`, the adjacent-question workaround hard-fails `app-release-qa`'s constraint-locality check, and `connect_set_verification_flags` REFUSES `gps` / `gps_radius_meters` with a typed `unsupported_verification_flag` (ace#1006, ace#1013). So every location-bound opportunity was deducted 25%-weighted for the absence of a control no build can produce, with no journey set able to score above the 6.0–7.0 band on that sub-anchor. Measured on `hh-poverty-targeting/20260824-1404`, whose PDD § 7.4 states plainly that accuracy is observability and whose `open-questions.md` item 3 forbids picking a radius: the judge scored 7.0 and wrote *"this journey set demands that GPS accuracy NOT be gated … the rubric grades what the app is made to demand, not the quality of the reasoning for demanding less."* Anchor (b) now demands the ACHIEVABLE contract — accuracy read back on screen, retry affordance, `gps_accuracy_m` submitted with every outcome, an advisory whose branches include an above-tolerance one — and states that an advisory-rather-than-block scores at the TOP of the anchor. A Table-A/Table-B check now gates every (a)–(d) deduction, which is the class-level half. Anchor (d) was checked too, and the issue's proposal to generalise the fix to every refused verification flag is **refuted**: the retired `duplicate` FLAG is one surface among several and does not by itself make duplicate handling unbuildable. A first draft of this rewrite then over-corrected in the opposite direction — asserting duplicate detection IS buildable on Connect's `entity_id` grain — and a blind re-grade of the same artifact immediately deducted on that new, uncited capability claim, i.e. the fix had reproduced the defect it was closing. Withdrawn: (d) now names both cautions and routes through the same Table-A/Table-B check, which is why the check is stated once for all of (a)–(d) rather than inline in (b). *Enforced:* `test/skills/pdd-must-not-assert-mechanisms.test.ts` § "no eval rubric demands a Table-A mechanism". | ACE team |
