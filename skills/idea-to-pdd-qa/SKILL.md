...
|---|---|---|---|---|
| 0 | `launch_parameters_present` | static | Every parameter Phase 3 (app build) and Phase 4 (opportunity creation) needs is declared in § Program Parameters: FLW pay per visit (`amount`), **LLO pay per visit (`org_amount`)**, daily target (`max_daily`), per-FLW cap (`max_total`), campaign target, `total_budget`, opportunity start/end dates, verification flags, and the **program shell** — `connect_delivery_type`, `opportunity_country`, `opportunity_currency`. `[PROPOSED]` / `[TBD]` values are ACCEPTED — the check enforces that the parameter was decided and written where Phase 4 looks, not that a human signed it off. **Country and currency are additionally checked for COHERENCE** (`lib/program-locale.ts`): an opportunity's amounts are always in the local currency of its country, so `IND` + `USD` fails with both rows present, an unrecognised country halts rather than defaulting, and a `[PROPOSED]` marker does not exempt the pair. The shell trio is create-time-only on a Connect program — `connect_update_program` changes none of the three — so this is the last cheap place to catch them. **Runs first on purpose.** | add a row per missing key to the § Program Parameters table; if a figure is unsettled write the proposal (`llo_payment_per_visit \| 1.50 [PROPOSED]`) with a matching `decisions.yaml` row carrying `evidence_basis: inferred`. Omitting the row is the only wrong answer. For an incoherent locale, set the currency to the country's own money — never resolve it by defaulting to USD |
