...
| 2 | `required_keys_present` | static | Top-level required keys present: `opportunity_id`, `random_seed`, `timeline`, `flw_personas`, `beneficiary_cohorts`, `kpi_config` | re-emit the manifest with the missing top-level keys (default values OK; eval grades quality separately) |
| 3 | `flw_personas_well_formed` | static | `flw_personas` is a non-empty array; each item has `id` (or `display_name`) + `archetype`; archetype value is in {rockstar, steady, struggling, new_hire} | regenerate flw_personas with required fields + valid archetypes; default mix is 1 rockstar / 2 steady / 1 struggling / 1 new_hire |
...
| 5 | `anomalies_traceable` | static | `anomalies` (when present) is an array; each anomaly has `flw_id`, `week`, and one of `field_path` / `detection_path` | populate detection_path + flw_id + week per anomaly; an anomaly without detection is reviewer-invisible downstream |
...
| 7 | `random_seed_present` | static | `random_seed` is a positive integer (deterministic generation requirement) | set `random_seed:` to today's date as YYYYMMDD |
