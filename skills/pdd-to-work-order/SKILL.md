...
5. **Render the work-order template to a Google Doc.**
   ...
   - **After the copy returns, call `docs_finalize_bullets(documentId=<new-doc-id>)` (best-effort).** This atom applies real Google Docs bullet styling to paragraphs enclosed in `<<<BULLETS_<NAME>_START>>>` / `<<<BULLETS_<NAME>_END>>>` anchor pairs, deletes the anchors, and returns the count of pairs processed. **The shipped `WORK_ORDER_TEMPLATE_ID` template currently has ZERO `<<<BULLETS_*>>>` anchor pairs** (it uses plain `{{..._body}}` tokens instead), so this call processes **0 anchors and is a no-op** today — that is expected and NOT a failure. The bulleted regions (§2 scope, §4.2 verified-unit criteria, §4.3 reporting, §8.1 permissions) still render correctly as content; they just appear as plain `\n`-separated paragraphs rather than Google-Docs-styled bullets. Always call it anyway: it's idempotent, harmless on a no-anchor doc, and becomes a no-op→active styling pass automatically once the anchors are added (no skill change needed). A `processed: 0` result is success, not an error to halt on. (jjackson/ace#614.)
   - **Durable fix lives in the external Drive template, not in this skill.** To get real bullet styling, the four `{{..._body}}` regions in `WORK_ORDER_TEMPLATE_ID` need to be wrapped in matching anchor paragraphs — e.g. a `<<<BULLETS_SCOPE_START>>>` paragraph immediately before `{{scope_body}}` and `<<<BULLETS_SCOPE_END>>>` immediately after (and likewise `VERIFIED_UNIT` / `REPORTING` / `PERMISSIONS`). That edit happens in the Google Doc template itself (it can't be made in this repo); once it lands, the `docs_finalize_bullets` call above starts styling those regions with no skill change. Tracked in jjackson/ace#614.
   - Body tokens for bulleted regions (`{{scope_body}}`, `{{verified_unit_body}}`, `{{reporting_body}}`, `{{permissions_body}}`) take a `\n`-separated string with one bullet item per line. `replaceAllText` honors `\n` as paragraph breaks; once the template carries anchor pairs, `docs_finalize_bullets` bullet-styles each resulting paragraph.
   ...
   **Bulleted-region tokens (newline-separated; styled via `docs_finalize_bullets` only once the template carries the anchor pairs — see Step 5; today they render as plain paragraphs):**
