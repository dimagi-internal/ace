...
9. **Write the run summary** to
   ...
   - **Render deep-links (with `run_id` — this is what downstream skills
     ...
     - LLO Weekly Review: `${LABS_BASE_URL}/labs/workflow/<llo_weekly_review_id>/run/?run_id=<latest run_id = Week 2, else Week 1>&opportunity_id=<labs_opp_id>`
     - Program Admin Audit: `${LABS_BASE_URL}/labs/workflow/<program_admin_audit_id>/run/?run_id=<audit_run_id>&opportunity_id=<labs_opp_id>`
