...
9. **Write the run summary** to
   ...
   - **Render deep-links (with `run_id` — this is what downstream skills
     screenshot/link):** the bare `/labs/workflow/<id>/?opportunity_id=<opp>`
     URL renders the run *picker*, not the polished per-FLW dashboard — the
     completed render only mounts when a saved `run_id` is in the URL. So
     emit, for each workflow that saved a run:
     - LLO Weekly Review: `${LABS_BASE_URL}/labs/workflow/<llo_weekly_review_id>/run/<latest run_id = Week 2, else Week 1>/?opportunity_id=<labs_opp_id>`
     - Program Admin Audit: `${LABS_BASE_URL}/labs/workflow/<program_admin_audit_id>/run/<audit_run_id>/?opportunity_id=<labs_opp_id>`
     `synthetic-summary`, `synthetic-workflow-polish-eval` (step 6 capture),
     and `synthetic-walkthrough-spec` all consume these. Recipe verified
     live against labs prod (jjackson/ace#769; jjackson/connect-labs#541).
