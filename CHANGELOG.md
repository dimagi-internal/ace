...
Phase 4 (`connect-setup`) can create a Connect opportunity, but only from a PDD, a Phase-3 deploy summary and a `run_state.yaml`, writing verdicts and phase-products back to Drive. That coupling is derivation and bookkeeping, not logic: the Connect-side sequence underneath is ~10 atom calls over ~15 values, and `connect-opp-setup` has no non-run path (it hard-halts without a PDD Evidence Model).

- **`skills/connect-opp-create`** (slash-command only) — validate → resolve program → application pre-flight → mint fresh app copies → create → payment units → verification flags → activate + deferred read-back → invites + invite read-back → local record. It defers to `connect-opp-setup` as the reference for every Connect-side gotcha rather than re-deriving them.
- **`--clone <opp>`** hydrates a spec from a live opportunity and **stops**, deliberately leaving the app ids blank. Hydrate and create are two commands because the useful half of a clone is the prose and the shape, and the half you must re-mint is the wiring. The hydrated spec records what is *not* recoverable rather than papering over it: payment-unit `amount`/`max_total` (ace#1642), the program UUID (no read surface carries it), verification flags (no read atom exists — only `set`), and `required_deliver_units`.
- **`scripts/validate-connect-opp-spec.ts` + `lib/connect-opp-spec.ts`** — the Step 1 gate. Takes `unknown`, because its input is `yaml.parse()` of a hand-authored file, and returns every issue at once rather than throwing on the first (a create/fix/create loop is not free — the create registers HQApiKey records, fetches app names from HQ, and is write-once).
- **`templates/connect-opp-spec.yaml`** — the single copy of the spec format; the skill points at it instead of restating it.

**The clone traps are code, not prose.** Reusing the source opportunity's Deliver app makes payment units impossible (Connect keys `DeliverUnit` on the released app, there is no delete atom, and an opp with no payment unit can never activate — ace#573); reusing its Learn app silently discards the posted `passing_score` (`get_or_create` with `update_existing=False` on a row keyed by app, not opportunity — ace#1350). Both are invisible until late and neither is recoverable in place, so the validator blocks them and Step 2 re-checks live against the apps the program's other opportunities already use — the provenance check alone cannot cover a hand-written spec. Fresh ids come from `commcare_linked_app_copy`, live-validated 2026-09-01 on `connect-ace-prod`: master→child with `linked: true`, or same-domain `linked: false` for HQ's plain "Copy Application", which preserves the `appearance="acquire"` and `display_style` settings a Nova re-upload wipes (ace#1643).

**Three fixes that outlive this feature**, all found by review of its first revision:

- **The capacity guard failed OPEN on non-finite input, on both sides.** `numberOfUsers(...) < 1` is `false` for `NaN`, so a quoted `total_budget: "900"` or a string `amount` sailed through — in the new validator *and* in `assertFundsAtLeastOneUser`, the MCP boundary guard that exists because this check already failed twice as prose (ace#722, ace#729). The shared invariants now live in **`lib/connect-opp-invariants.ts`** and `mcp/connect/opportunity-{name,capacity}.ts` import them, so there is one definition instead of a copy pinned by a test — a pin detects drift, and these two had already drifted on the case that matters. `fundsAtLeastOneUser` treats non-finite as a failure.
- **`test/no-mojibake.test.ts`** — double-encoded UTF-8 is now a CI failure. Three new files had picked up 68 occurrences of it (PowerShell reading UTF-8 as cp1252), it had reached operator-facing validator strings and agent-facing `§` references, and one had become a test's *expected value* — at which point fixing the source turns the suite red and the cheap move is to re-corrupt it.
- **`validateConnectOppSpec` is enrolled in the negative-control ratchet.** Its first revision passed every positive test while failing open on malformed input, which is precisely the gap a negative control closes.
...
**Geopoint:** #593 anticipated an accuracy-gated native geopoint widget needing a mock-location fix. Live, the RDT Sample "GPS location" question rendered as a plain `android.widget.EditText` (free text), so this was documented as "a free-text field by design — type a `lat lon alt accuracy` string." **CORRECTED 2026-06-03 (jjackson/ace#686): that conclusion was wrong.** The field IS a real CommCare geopoint (`kind:geopoint`; the form carries the standard `selected-at(gps, 0|1|3)` calcs). The plain-text-box render came from a **stale/downgraded build** whose XForm bind compiled to `type="xsd:string"` instead of `type="geopoint"`; a fresh compile of the same Nova app yields `type="geopoint"` (Nova is not at fault). Typing a string into the stale widget is what produced the runtime `Calculation Error … gps_lon … list with only 1 element`. A correct build renders the native Capture-button widget (needs `adb emu geo fix` before tapping Capture). Preventer added in `app-release-qa` (kind:geopoint MUST compile to bind `type="geopoint"`); recipe-authoring rule in `app-test-cases` Step 3 item 4.5; the selector-map NOTE is corrected accordingly.
...
No semantic changes — same writers, same readers, same shape. Pure textual rename of every `outputs:` YAML key and every `outputs.*` dotted reference in `agents/`, `skills/`, `docs/`, `lib/artifact-manifest.ts`, and `CLAUDE.md`. Plus the corresponding rename of the `## Outputs` SKILL.md section heading (62 skills) and the `**Outputs:**` prose label in agent phase summaries — same word, same chain, so the section heading was renamed to `## Products` for consistency with the YAML key the section references. The skills/README.md convention doc was updated to match. Test fixtures that used "outputs" only as a generic English noun were not touched.
