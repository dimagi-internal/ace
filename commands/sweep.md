---
description: Sweep orphaned ACE artifacts across drive, connect, ocs, hq, labs; or prune per-opp run history via opp-runs
...
- `<system>` (optional) — one of `drive`, `connect`, `ocs`, `hq`, `labs`, `opp-runs`, `all`. Omit to be prompted.
...
/ace:sweep all          # runs drive + connect + ocs + hq + labs in sequence (opp-runs excluded — retention is a manual decision)
```
